...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char(AT_YELLOW, buf, ch );
    sprintf( buf,
	    "TSR awoke at %s\rThe system time is %s\r",
	    str_boot_time,
...
if IS_IMMORTAL( ch )
{
    ...
    {
      sprintf( buf, "TSR compiled at %s\r", (char*)ctime(&statis.st_mtime ) );
      send_to_char( AT_RED, buf, ch );
    }
    else
    {
...
/*    fGuildRestrict = FALSE;  */
	...
	if ( is_number( arg ) )
	{
	    ...
	    {
              default:
		send_to_char(AT_DGREEN, "This feature is no longer with us.\n\r", ch );
		return;
	    }
	}
	else
...
"&R           T&C:&RH&C:&RO&C:&RS&C:&RE&C: :&RI&C:&RN&C: :&RT&C:&RH&C:&RE&C: :&RS&C:&RH&C:&RA&C:&RD&C:&RO&C:&RW&C: :&RR&C:&RE&C:&RA&C:&RL&C:&RM&C:&RS         \n\r",ch);
    send_to_char( AT_RED, 
"&C           ^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^V^         \n\r\n\r", ch); 
...
/*	  wch->name, ( *wch->pcdata->lname != 39 ) ? " " : "",  */
	  ...
	  sprintf( buf + strlen( buf ), "&W[&R%s&G%s&W] %s%s%s &w%s%s ", 
            IS_SET(wch->act, PLR_PKILLER) ? "K" : " ",
            IS_SET(wch->act, PLR_SOUND) ? "S" : " ",
            IS_SET(wch->act, PLR_AFK) ? "&W<&YAFK&W>" : "",
            IS_SET(wch->act, PLR_WIZINVIS) ? "&W(&RWizi&W)" : "",
            IS_SET(wch->act, PLR_CLOAKED) ? "&W(&RCloaked&W)" : "",
  ...
  if ( pMatch >= 0 || iMatch >= 0 )
  {
    ...
    if (max_on == count)
      sprintf(buf3, "&wTotal on Today: &R%d\n\r", count);
    else
      sprintf(buf3, "&wTotal on Today: &R%d\n\r", max_on);
    send_to_char(AT_WHITE, buf3, ch );  
  }
  else
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(AT_PINK, !IS_SET( ch->deaf, CHANNEL_GOSSIP     )
		     ? " +GOSSIP"
		     : " -gossip",
		     ch );
	...
	send_to_char(AT_PINK, !IS_SET( ch->deaf, CHANNEL_CLASS )
        ...
        send_to_char(AT_PINK, !IS_SET( ch->deaf, CHANNEL_DEV )
                     ? " +DEV"
                     : " -dev",
                     ch );
/* 	if( ch->level > 2 )
...
*/
	...
	else
	{
        ...
        else if ( !str_cmp( arg+1, "chat"     ) ) bit = CHANNEL_GOSSIP;
        ...
        else if ( !str_cmp( arg+1, "clan"     ) ) bit = CHANNEL_DEV;
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(AT_RED,  IS_SET( ch->act, PLR_SOUND     )
	    ? "&w[&Y+SOUND     &w]&W You have MSP Sounds enabled.\n\r"
	    : "&w[&Y-sound     &w]&W You have MSP Sounds disabled.\n\r"
	    , ch );
...
/* 
 * We now have sounds in the MUD
 */
void do_sound ( CHAR_DATA *ch, char *argument )
{
    char buf[ MAX_STRING_LENGTH ];

    ( IS_SET ( ch->act, PLR_SOUND )
     ? sprintf( buf, "-sound" )
     : sprintf( buf, "+sound" ) );
...
void do_sects( CHAR_DATA *ch, char *argument )
{
    CLAN_DATA    *pClan;
    char          buf[MAX_STRING_LENGTH];
