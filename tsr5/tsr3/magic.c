...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( ch ) && ( ch->level < LEVEL_MORTAL ) )
    {
       ...
       if ( prime_class( ch ) == CLASS_VAMPIRE )
       		mana /= 4;
    ...
    if ( !IS_NPC( ch ) )
    if ( ( prime_class( ch) != CLASS_VAMPIRE ) && ch->mana < mana )
       {
   	send_to_char(AT_BLUE, "You don't have enough mana.\n\r", ch );
	return;
       }
    else
       if ( ( ch->bp < mana ) && ( prime_class( ch ) == CLASS_VAMPIRE ) ) 
       {
...
void spell_resonate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    OBJ_DATA *obj = (OBJ_DATA *) vo;

    if ( obj->extra_flags == ITEM_SHARD )
    {
	    send_to_char(AT_GREEN, "Something begins to vibrate.\n\r", ch );
    }
    else
    {
	send_to_char(AT_GREEN, "Nothing happends.\n\r", ch );
    }
    return;
}
