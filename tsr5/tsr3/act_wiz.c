...
void do_ideas( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !( fp = fopen( "/home/beowolf/realms/area/ideas.txt", "r" ) ) )
   return;
...
void do_typos( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !( fp = fopen( "/home/beowolf/realms/area/typos.txt", "r" ) ) )
   return;
...
void do_bugs( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !( fp = fopen( "/home/beowolf/realms/area/bugs.txt", "r" ) ) )
   return;
...
void do_wizhelp( CHAR_DATA *ch, char *argument )
{
  ...
  for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
  {
    ...
    &&   cmd_table[cmd].level <= get_trust( ch ) )
    {
      ...
      {
	case (MAX_LEVEL -11):tile = "p"; break;
	case (MAX_LEVEL -10):tile = "P"; break;
	case (MAX_LEVEL - 9):tile = "g"; break;
	case (MAX_LEVEL - 8):tile = "G"; break;
	case (MAX_LEVEL - 7):tile = "W"; break;
	case (MAX_LEVEL - 6):tile = "Y"; break;
	case (MAX_LEVEL - 5):tile = "b"; break;
	case (MAX_LEVEL - 4):tile = "B"; break;
	case (MAX_LEVEL - 3):tile = "c"; break;
	case (MAX_LEVEL - 2):tile = "C"; break;
	case (MAX_LEVEL - 1):tile = "r"; break;
	case       MAX_LEVEL:tile = "R"; break;
      }
...
void do_goto( CHAR_DATA *ch, char *argument )
{
    ...
    act( AT_RED, "$t", ch, ch->pcdata->bamfusee, NULL, TO_CHAR );
	
    if ( !IS_SET( ch->act, PLR_WIZINVIS ) )
    {
        act( AT_RED, "$t", ch, ch->pcdata->bamfout, NULL, TO_ROOM );
	if ( pet )
    ...
    if ( !IS_SET( ch->act, PLR_WIZINVIS ) )
    {
        act( AT_RED, "$t", ch, ch->pcdata->bamfin, NULL, TO_ROOM );
    }
    else
     {
       for ( gch = ch->in_room->people; gch; gch = gch->next_in_room )
...
void do_ostat( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Wear bits: %d.  Extra bits: %s.\n\r",
	    obj->wear_flags, immextra_bit_name( obj->extra_flags ) );
    send_to_char(AT_RED, buf, ch);
/* FOR NEW FLAGS */
	...
	if (!str_cmp( arg3, "poisoned" ) || !str_cmp( arg3, "p" ) )
	  ...
	  value = ITEM_FLAME;
        ...
        if (!str_cmp( arg3, "frosty" ) || !str_cmp( arg3, "f" ) )
          value = ITEM_ICY;
        if (!str_cmp( arg3, "shard" ) || !str_cmp( arg3, "sh" ) )
          value = ITEM_SHARD;
...
char *con_type_name( int connected )
{
  ...
  if ( connected == CON_CHECK_SOUND ) return "Checking MSP Sound";
  if ( connected == CON_NAME_CONTRACT ) return "Name check";
  if ( connected == CON_RULE_ACCEPT ) return "Rules";
  return "Unknown";
}
...
void do_sockets( CHAR_DATA *ch, char *argument )
{
    ...
    count	= 0;
    buf[0]	= '\0';
    strcpy( buf, "[Desc Connected  ] Name&W@&GHost&R\n\r" );
	...
	{
	    count++;
	    sprintf( buf + strlen(buf), "[%4d %-11s] %s&W@&G%s&R\n\r",
		    d->descriptor,
...
void do_rank( CHAR_DATA *ch, char *argument )
{
  ...
  if ( argument[0] == '\0' )
    {
    send_to_char( C_DEFAULT, "Change your rank to what?\n\r", ch );
    return;
    }
  ...
  if ( !str_cmp( "default", argument ) )
	...
	ch->pcdata->whotype = str_dup( "A Dumbass" );
	}
  else
	{
	free_string( ch->pcdata->whotype );
...
void do_nuke( CHAR_DATA *ch, char *argument )
{
  ...
  argument = one_argument( argument, arg );
   ...
   act( AT_RED, 
      "A rift opens beneath $n, who falls into oblivion.", vch, NULL, NULL, TO_ROOM );
   act( AT_RED, 
      "A rift opens beneath you, you fall into oblivion.", vch, NULL, NULL, TO_VICT );
...
void do_allow( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( AT_RED, "Remove which site from the ban list?\n\r", ch );
	return;
    }
    ...
    send_to_char( AT_RED, "Site is not banned.\n\r", ch );
    return;
}
...
guild_table[iGuild].name ) )   */
	...
	if ( IS_SET( wch->act, PLR_PKILLER ) )
	  send_to_char( C_DEFAULT, "&W[&RK&W] ", ch );
	buf[0] = '\0';
...
/*
 * Sound to all characters.
 */
void sound_to_all( const char *text )
{
    DESCRIPTOR_DATA *d;
    ...
    if ( !text )
        return;
    for ( d = descriptor_list; d; d = d->next )
        if ( d->connected == CON_PLAYING && IS_SOUND(d->character)) 
