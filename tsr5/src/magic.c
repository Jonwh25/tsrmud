...
void magic_mob ( CHAR_DATA *ch, OBJ_DATA *obj, int vnum )
{
   ...
   char            buf [MAX_STRING_LENGTH];
    ...
    update_pos( zombie );
    act(  "$n passes $s hands over $p, $E slowly rises to serve $S new master.", ch, obj, zombie, TO_ROOM );
    act(  "You animate $p, it rises to serve you.", ch, obj, NULL, TO_CHAR );
...
void do_acspell ( CHAR_DATA *ch, OBJ_DATA *pObj, char *argument )
{
    ...
    if ( ( sn == spec )  && ( is_name( ch, arg2, ch->name ) ) )
       {
         send_to_char(  "You are already in the same room as yourself.\n\r", ch );
         return;
       }
...
/*
	...
	{
	 send_to_char( "The sun blocks your magic!\n\r", ch );
	 return;
	}
      }
    }
*/
    ...
    {
      if ( prime_class(ch) != CLASS_BARD )
      send_to_char( "You're too stunned to cast spells.\n\r", ch );
      else
      send_to_char( "You're too stunned to sing songs.\n\r", ch );
      return;
    }
	...
	if ( arg2[0] != '\0' && !is_name( ch, arg2, ch->name ) )
	{
	    ...
	    send_to_char( "You cannot sing this song for others.\n\r", ch );
	    return;
	}
	...
	return;
       ...
       {
   	send_to_char( "You are to starved to cast, you must feed.\n\r", ch );
	return;
       }
	...
	if ( ( IS_AFFECTED2( ch, AFF_CONFUSED ) )
	    && number_percent( ) < 10 )
	{
	   act( "$n looks around confused at what's going on.", ch, NULL, NULL, TO_ROOM );
	   send_to_char(  "You become confused and botch the spell.\n\r", ch );
	   return;
	} 
	update_skpell( ch, sn );   
		    ...
		    OBJ_DATA *obj )
{
    ...
    if ( ( ( ch->level - 9 > victim->level ) 
      ...
      && ( !IS_NPC(victim) ) )
    {
	send_to_char( "That is not in the pkill range... valid range is +/- 8 levels.\n\r", ch );
	return;
    }
	vo = (void *) victim;
	break;
...
void spell_astral( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( "$n vanishes in a flash of blinding light.", ch, NULL, NULL, TO_ROOM );
    if ( ch != victim )
    {
     if ( pet )
     {
       act(  "$n vanishes in a flash of blinding light.", pet, NULL, NULL, TO_ROOM );
       char_from_room( pet );
     }
     char_from_room( ch );
     char_to_room( ch, victim->in_room );
    }
    act( "$n appears in a flash of blinding light.", ch, NULL,
NULL, TO_ROOM );
    do_look( ch, "auto" );
    if ( pet )
    {
      char_to_room( pet, victim->in_room );
      act(  "$n appears in a flash of blinding light.", pet, NULL, NULL, TO_ROOM );
    }
    return;
}
...
void spell_pass_plant ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
      send_to_char(  "You cannot walk through the plants in your condition!\n\r", ch );
      return;
    }
    ...
    {
	send_to_char( "There are no plants here.\n\r", ch );
	return;
    }

    if ( IS_SET( ch->in_room->room_flags, ROOM_NO_SHADOW ) )
    {
        send_to_char( "The plants do not respond to your call.\n\r", ch);
        return;
    }
	...
	|| IS_SET( victim->in_room->room_flags, ROOM_SOLITARY  )
        ...
        || IS_AFFECTED( victim, AFF_NOASTRAL )  )
    {
        send_to_char( "The plants do not reach there.\n\r", ch );
        return;
    }
    ...
    if ( ch != victim )
    {
     act( "$n melts into the surrounding plant life.", ch, NULL, NULL, TO_ROOM );
     char_from_room( ch );
     char_to_room( ch, victim->in_room );
     act( "$n steps out of the surrounding plant life.", ch, NULL, NULL, TO_ROOM );

     if ( pet )
     {
      act(  "$n melts into the surrounding plant life.", pet, NULL, NULL, TO_ROOM );
      char_from_room( pet );
      char_to_room( pet, victim->in_room );
      act(  "$n steps out of the surrounding plant life.", pet, NULL, NULL, TO_ROOM );
     }
    }
...
void spell_aura( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You feel a wave of peace flow lightly over your body.\n\r", victim );
    act( "$n looks very peaceful.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_darkbless( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    affect_to_char( victim, &af );
    if ( ch != victim )
	send_to_char( "You call forth the hand of oblivion.\n\r", ch );
    send_to_char( "The hand of oblivion rests upon you.\n\r", victim );
    return;
}
...
void spell_aid( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch != victim )
	send_to_char( "You grant them divine aid.\n\r", ch );
    send_to_char( "You feel divine aid reashure you.\n\r", victim );
    return;
}
...
void spell_bio_acceleration( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char(  "You greatly enhance your bio-functions.\n\r", ch );
...
void spell_draw_strength( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char(  "You draw from your physical strength and
increase your energy reserve.\n\r", ch );
    act( "$n's body weakens.", ch, NULL, NULL, TO_ROOM);
    return;
}
...
void spell_blood_bath( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( blood_count( ch->in_room->contents, 5 ) < 2 )
       {
        send_to_char(  "There is not enough blood in the room.\n\r", ch );
	return;
       }
	...
	victim->hit = UMIN( victim->hit + 250, MAX_HIT(victim) );
        update_pos( victim );
        act(  "You bath $N in the life giving fluid.", ch, NULL, victim, TO_CHAR );
	act(  "$n baths $N in blood.", ch, NULL, victim, TO_ROOM );
	act(  "$n baths you in blood.", ch, NULL, victim, TO_VICT );
    }
    send_to_char(  "The blood bath is over.\n\r", ch );
    return;
}
...
void spell_call_lightning( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    dam = dice( level / 2, 8 );
    send_to_char( "Lightning slashes out of the sky to strike your foes!\n\r", ch );
    act( "$n calls lightning from the sky to strike $s foes!",
...
void spell_create_bard( int sn, int level, CHAR_DATA *ch, void *vo )
{
    OBJ_DATA *mushroom;
...
void spell_dispel_magic ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	    ...
	    && !saves_spell( level, victim ) )
	{
	    ...
	    send_to_char( "The flames about your body have been doused.\n\r",
								victim );
	    act( "The flames about $n's body burn out.",
			victim, NULL, NULL, TO_ROOM );
	}
	    ...
	    && !saves_spell( level, victim ) )
	{
	    ...
	    send_to_char( "The electricity about your body flee's into the ground..\n\r",
								victim );
	    act( "The electricity about $n's body flee's into the ground.",
			victim, NULL, NULL, TO_ROOM );
	}
	    ...
	    && !saves_spell( level, victim ) )
	{
	    ...
	    send_to_char( "The icy crust about your body melts to a puddle..\n\r",
								victim );
	    act( "The icy crust about $n's body melts to a puddle.",
			victim, NULL, NULL, TO_ROOM );
	}
...
void spell_chain_lightning( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "Bolts of electricity arc from your hands!\n\r", ch );
	...
	if ( vch->in_room->area == ch->in_room->area )
	    send_to_char( "The air fills with static.\n\r", vch );
    }
...
void spell_meteor_swarm( int sn, int level, CHAR_DATA *ch, void *vo )
{
   ...
   AFFECT_DATA af;
    ...
    send_to_char( "Flaming meteors fly forth from your outstreched hands!\n\r", ch );
	...
	if ( vch->in_room == ch->in_room )
	{
		...
		damage( ch, vch, level + dice( level, 8 ), sn );
            if ( vch != ch )
            {
                ...
                affect_join( vch, &af );
	        send_to_char( "You body bursts into flame!\n\r", vch);
	     }
	    continue;
	}
...
void spell_holysword( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	|| ( obj->affected && !IS_OBJ_STAT( obj, ITEM_DWARVEN ) ) )
    {
	send_to_char( "That item cannot be consecrated.\n\r", ch );
	return;
    }
...
void spell_psychic_quake( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You let the chaos free from your mind!\n\r", ch );
    act( "$n's face becomes blank and concentrated.", ch, NULL, NULL, TO_ROOM );
	...
	if ( vch->in_room->area == ch->in_room->area )
	    send_to_char( "A wave of chaos brushes your mind.\n\r", vch );
    }
...
void spell_fireshield( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "Your body is engulfed by unfelt flame.\n\r", victim );
...
void spell_eternal_intellect( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
    send_to_char( "You feel an unsurpased intelligence.\n\r", victim );
    return;
}
void spell_golden_aura( int sn, int level, CHAR_DATA *ch, void *vo )
{
   ...
   affect_to_char2( victim, &af );
   send_to_char(  "You are surrounded by a golden aura.\n\r", victim );
...
void spell_goodberry( int sn, int level, CHAR_DATA *ch, void *vo )
{
   ...
   OBJ_DATA     *berry;    
    ...
    act( "You pass your hand over $p slowly.", ch, obj, NULL, TO_CHAR );
    act( "$n has created a goodberry.", ch, NULL, NULL, TO_ROOM );
...
void spell_iceshield( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "An Icy crust forms about your body.\n\r", victim );
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( obj->ac_type != 0 )
    {
      switch( obj->ac_type )
      {
       default:  send_to_char( "Invoke Type Unknown.\n\r", ch ); break;
       case 1 :
         {
	   ...
	   {
	    send_to_char( " '", ch );
	    send_to_char( spn ? obj->ac_spell : "(none)", ch );
	    send_to_char( "'\n\r", ch );
	   }
	   break;
         }
...
void spell_vibrate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You set up a complex set of vibrations around your body.\n\r", victim );
    act( "$n's body begins to vibrate.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_incinerate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch != victim )
	send_to_char( "Ok.\n\r", ch );
    send_to_char( "Your body bursts into flames!\n\r", victim );
    return;
}
...
void spell_phase_shift( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You phase into another plane.\n\r", victim );
    act( "$n phases out of reality.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_mist_form( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You seem to feel transparent.\n\r", victim );
    act( "$n takes on the form of a mist.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
/*    AFFECT_DATA *paf;*/
    ...
    {
	send_to_char( "You cannot make that item permenant.\n\r", ch );
	return;
    }
    obj->value[2] = -1;
    obj->value[1] = -1;
    act( "You run your finger up $p, you can feel it's power growing.", ch, obj, NULL, TO_CHAR );
    act( "$n slowly runs $s finger up $p.", ch, obj, NULL, TO_ROOM );
    return;
}
...
void spell_polymorph( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  char                buf [MAX_STRING_LENGTH]; 
   ...
   if ( !(victim = get_char_world( ch, target_name ) )
    ...
    act( "$n's form wavers and then resolidifies.", ch, NULL, NULL, TO_ROOM);
    send_to_char( "You have succesfully polymorphed.\n\r", ch );
    return;
}
void spell_portal( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( "A huge shimmering gate rises from the ground.", ch, NULL, NULL, TO_CHAR );
    act( "$n utters a few incantations and a gate rises from the ground.", ch, NULL, NULL, TO_ROOM );
    obj_to_room( gate1, ch->in_room );
    act( "A huge shimmering gate rises from the ground.", victim, NULL, NULL, TO_CHAR );
    act( "A huge shimmering gate rises from the ground.", victim, NULL, NULL, TO_ROOM );
...
void spell_remove_curse( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !( obj = get_eq_char( victim, iWear ) ) )
        ...
        if ( IS_SET( obj->extra_flags, ITEM_NODROP ) )
        {
            ...
            send_to_char(  "You feel a burden relieved.\n\r", ch );
            yesno = TRUE;
        }
    ...
    {   
        ...
        send_to_char( "Your body has been extinguished.\n\r", ch);
        yesno = TRUE;
    }    
...
void spell_web( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( "$n has been imobilized by a plethora of sticky webs.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
if ( saves_spell( level, victim ) )
   {
    ...
    send_to_char( buf, victim );
    act( "$n stares around blankly.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
if ( saves_spell( level, victim ) )
   {
    ...
    send_to_char( buf, victim );
    act( "$n looks very clumsy.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_mind_probe( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( victim->level >= LEVEL_IMMORTAL )
    {
      ...
      send_to_char( "The mind of an immortal is beyond your understanding.", ch );
      return;
    }

    sprintf(buf1, "You send your conciousness into %s's mind.\n\r", victim->name);
    send_to_char( buf1, ch );
    send_to_char( "You feel someone touch your mind.\n\r", victim );
    buf1[0] = '\0';
    
    if (IS_NPC(victim))
       {
         send_to_char ( "The mind is to chaotic to merge with.\n\r", ch );
         return;
       }
    sprintf( buf,
	    "You are %s%s.\n\r",
...
*/
    ...
    sprintf( buf,
	"Str: %d&p(%d&p)  Int: %d&p(%d&p)  Wis: %d&p(%d&p)  Dex: %d&p(%d&p)  Con: %d&p(%d&p).\n\r",
    ...
    send_to_char(  "You are ", ch );
         if ( GET_AC( victim ) >=  101 ) send_to_char(  "WORSE than naked!\n\r", ch );
    else if ( GET_AC( victim ) >=   20 ) send_to_char(  
"naked.\n\r"           , ch );
    else if ( GET_AC( victim ) >=    0 ) send_to_char(  "wearing 
clothes.\n\r" , ch );
    else if ( GET_AC( victim ) >= - 50 ) send_to_char(  "slightly 
armored.\n\r", ch );
    else if ( GET_AC( victim ) >= -100 ) send_to_char(  "somewhat 
armored.\n\r", ch );
    else if ( GET_AC( victim ) >= -250 ) send_to_char(  
"armored.\n\r"         , ch );
    else if ( GET_AC( victim ) >= -500 ) send_to_char(  "well 
armored.\n\r"    , ch );
    else if ( GET_AC( victim ) >= -750 ) send_to_char(  "strongly 
armored.\n\r", ch );
    else if ( GET_AC( victim ) >= -1000 ) send_to_char(  "heavily 
armored.\n\r" , ch );
    else if ( GET_AC( victim ) >= -1200 ) send_to_char(  "superbly 
armored.\n\r", ch );
    else if ( GET_AC( victim ) >= -1400 ) send_to_char(  "divinely 
armored.\n\r", ch );
    ...
    send_to_char(  "The presence lifts from your mind.\n\r", victim );
    return;
}
...
void spell_entangle( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( buf, victim );
    act( "Hundreds of vines reach from the ground to entangle $n.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_shockshield( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "Sparks of electricity flow into your body.\n\r", victim );
    act( "Bolts of electricity flow from the ground into $n's body.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_spell_bind( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
	send_to_char( "You cannot bind magic to that item.\n\r", ch );
	return;
    }
	
    if (!Charged)
    {
        send_to_char( "That item is at full charge.\n\r", ch );
        return;
    }

    act( "You slowly pass your hand over $p, it vibrates slowly.", ch, obj, NULL, TO_CHAR );
    act( "$n slowly passes $s hand over $p, it vibrates slowly.", ch, obj, NULL, TO_ROOM );
    return;
}
...
void spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char(  "You feel a wave of nausia come over you.\n\r", ch );
...
void spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( "$n glimmers briefly, then is gone.", victim, NULL, NULL, TO_ROOM );
    if ( pet )
    {
      act(  "$n glimmers briefly, then is gone.", pet, NULL, NULL, TO_ROOM );
      char_from_room( pet );
    }
    char_from_room( victim );
    char_to_room( victim, pRoomIndex );
    act( "The air starts to sparkle, then $n appears from nowhere.",   victim, NULL, NULL, TO_ROOM );
    do_look( victim, "auto" );
    if ( pet )
    {
      char_to_room( pet, pRoomIndex );
      act(  "The air starts to sparkle, then $n appears from nowhere.", pet, NULL, NULL, TO_ROOM );
    }
    return;
}
...
void spell_chaosfield( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You call forth an instance of chaos from the order around you.\n\r", victim );
    act( "$n's body is veiled in an instance or pure chaos.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_bladebarrier( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You bring forth thousands of tiny spinning blades about your body.\n\r", victim );
    act( "$n's body is surrounded by thousands of spinning blades.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
void spell_dancing_lights( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( "&.Thou&.sand&.s &.of &.danci&.ng &.ligh&.ts &.surr&.ound &.you&.!", victim, NULL, victim, TO_VICT );
    act( "$n's &.body &.is &.surr&.ounded &.by d&.anci&.ng l&.ights.", victim, NULL, NULL, TO_ROOM );
    return;
}
...
/*    victim->hit = MAX_HIT(victim);*/
    ...
    if ( ch != victim )
        send_to_char( "Ok.\n\r", ch );
    send_to_char( "Ahhhhhh...You feel MUCH better!\n\r", victim );
...
void spell_disrupt( int sn, int level, CHAR_DATA *ch, void *vo )
{
       ...
       number_bits( 8 ) == 0 )
  {
    ...
    if ( ch == vch )
    {
      act(  "You disrupt yourself!", ch, NULL, NULL, TO_CHAR );
      act(  "$n disrupts $mself!", ch, NULL, NULL, TO_ROOM );
    }
    else
    {
      act(  "You disrupt $N!", ch, NULL, victim, TO_CHAR );
      act(  "$n disrupts $N!", ch, NULL, victim, TO_NOTVICT );
      act(  "$n disrupts you!", ch, NULL, victim, TO_VICT );
      switch( af.location )
      {
      case APPLY_MANA:
...
void spell_lend_health ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch == victim )
    {
	send_to_char( "Lend health to yourself?  Easily done.\n\r", ch );
	return;
    }
...
void spell_summon_swarm(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  act( "You summon $N.", ch, NULL, mob, TO_CHAR);
  ...
  MT( ch ) -= mana;
  act( "$n summons $N.", ch, NULL, mob, TO_ROOM);
  ...
  {
    act( "$n rescues you!", mob, NULL, ch, TO_VICT    );
    act( "$n rescues $N!",  mob, NULL, ch, TO_NOTVICT );
...
void spell_summon_pack(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  MT( ch ) -= mana;
  act( "$N comes to $n aid.", ch, NULL, mob, TO_ROOM);
...
void spell_summon_demon(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  ch->summon_timer = 16;
  char_to_room(mob, ch->in_room);
  act( "You summon $N from the abyss.", ch, NULL, mob, TO_CHAR);
  mana = is_class( ch, CLASS_VAMPIRE ) ? level / 2 : level * 2;
  if ( MT( ch ) < mana )
    {
    sprintf( buf, "%sYou don't have enough %s to bind $N!",
  ...
  MT( ch ) -= mana;
  act( "$n summons $N from the abyss.", ch, NULL, mob, TO_ROOM);
...
void spell_summon_angel(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  ch->summon_timer = 16;
  char_to_room(mob, ch->in_room);
  act( "You summon $N from heaven.", ch, NULL, mob, TO_CHAR);
  mana = is_class( ch, CLASS_VAMPIRE ) ? level / 2 : level * 2;
  if ( MT( ch ) < mana )
    {
    sprintf( buf, "%sYou don't have enough %s to bind $N!",
  ...
  MT( ch ) -= mana;
  act( "$n calls forth $N from Heaven.", ch, NULL, mob, TO_ROOM);
...
void spell_summon_shadow(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  ch->summon_timer = 16;
  char_to_room(mob, ch->in_room);
  act( "You summon $N from the shadow plane.", ch, NULL, mob, TO_CHAR);
  mana = is_class( ch, CLASS_VAMPIRE ) ? level / 2 : level * 2;
  if ( MT( ch ) < mana )
    {
    sprintf( buf, "%sYou don't have enough %s to bind $N!",
  ...
  MT( ch ) -= mana;
  act( "$n calls forth $N from the shadow plane.", ch, NULL, mob, TO_ROOM);
...
void spell_summon_trent(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  ch->summon_timer = 16;
  char_to_room(mob, ch->in_room);
  act( "You summon $N from the plane of nature.", ch, NULL, mob, TO_CHAR);
  mana = is_class( ch, CLASS_VAMPIRE ) ? level / 2 : level * 2;
  if ( MT( ch ) < mana )
    {
    sprintf( buf, "%sYou don't have enough %s to bind $N!",
  ...
  MT( ch ) -= mana;
  act( "$n calls forth $N from the plane of nature.", ch, NULL, mob, TO_ROOM);
...
void spell_summon_beast(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  ch->summon_timer = 16;
  char_to_room(mob, ch->in_room);
  act( "You call $N from the forests.", ch, NULL, mob, TO_CHAR);
  mana = is_class( ch, CLASS_VAMPIRE ) ? level / 2 : level * 2;
  if ( MT( ch ) < mana )
    {
    sprintf( buf, "%sYou don't have enough %s to bind $N!",
  ...
  MT( ch ) -= mana;
  act( "$n calls forth $N from the forests.", ch, NULL, mob, TO_ROOM);
...
/*	if(!saves_dispel(dis_level,victim->level,af->duration))*/
        {
          ...
          if(skill_table[sn].msg_off)
          {
            ...
            {
              act( skill_table[sn].room_msg_off,
	      victim, NULL, NULL, TO_ROOM);
            }
          }
	return TRUE;
...
/*
  ...
  {
    act( "$n's skin regains its normal texture.",
     victim,NULL,NULL,TO_ROOM);
    found = TRUE;
  }
...
void spell_turn_undead( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  {
    act("$n has turned $N!",ch,NULL,victim,TO_ROOM);
    act("You have turned $N!",ch,NULL,victim,TO_CHAR);
    raw_kill(ch,victim);
    return;
  }
...
void spell_mental_block( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  send_to_char(  "Your mind feels free of instrusion.\n\r",victim);
  if ( ch != victim )
    send_to_char( "Ok.\n\r",ch);
}
/* END */
...
void spell_holy_strength(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  if(ch != victim)
    send_to_char( "Ok.\n\r", ch );
  send_to_char( "The strength of the gods fills you.\n\r", victim);
  return;
}
...
void spell_curse_of_nature(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  if(ch != victim)
    send_to_char( "Ok.\n\r", ch );
  send_to_char( "The wrath of nature wrecks you.\n\r", victim);
  return;
}
...
void spell_enchanted_song(int sn, int level, CHAR_DATA *ch, void *vo)
{
  ...
  if ( ch == victim )
  {
    act(  "$n sings an enchanting song.", ch, NULL, NULL, TO_ROOM );
    send_to_char( "You sing a song.\n\r", ch );
  }
  act(  "Your song pacifies $N.", ch, NULL, victim, TO_CHAR );
  act(  "$n's song pacifies $N.", ch, NULL, victim, TO_NOTVICT );
  act(  "$n's song slows your reactions.", ch, NULL, victim, TO_VICT );
...
/*  divisos += 2;*/
  ...
  act( "You heal $N.", ch, NULL, victim, TO_CHAR);
  act( "$n heals you.", ch, NULL, victim, TO_VICT);
...
void spell_hex( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  && IS_AFFECTED2( victim, AFF_CONFUSED ) )
	{
	act(  "$N is already hexed.", ch, NULL, victim, TO_CHAR );
	return;
	}
  if ( saves_spell( level, victim ) )
	{
	act(  "$N resists the hex.", ch, NULL, victim, TO_CHAR );
	return;
	}
  ...
  send_to_char(  "You have hexed your opponent.\n\r", ch );
  send_to_char(  "A hex has been placed upon your soul.\n\r", victim );
  return;
}
...
void spell_dark_ritual( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  if ( found == TRUE )
    ...
    send_to_char(  "You extract the last of the energy from the corpse.\n\r", 
		  ch );
    act(  "$n saps away the last of the mystical energies from the $p.",
	 ch, obj, NULL, TO_ROOM );
    extract_obj( obj );
    }
  else
...
void spell_field_of_decay( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  send_to_char(  "You summon the power of the dead and a black haze envelops you.\n\r", ch );
  act(  "A black haze emanates from $n's body and envelops $m.",
       ch, NULL, NULL, TO_ROOM );
  return;
}
...
void spell_soul_bind( int sn, int level, CHAR_DATA *ch, void *vo )
{
   ...
   obj_to_char( soulgem , ch );
    ...
    act( "You tear out $Ns soul, binding it to form a Soulgem.", ch, NULL, victim, TO_CHAR);
    act( "$n tears out $Ns soul, binding it to form a Soulgem.", ch, NULL, victim, TO_ROOM);
    act( "$N screams in agony as it slowly dissipates into nothingness!", ch, NULL, victim, TO_CHAR);
    act( "$N screams in agony as it slowly dissipates into nothingness!", ch, NULL, victim, TO_ROOM);
    act( "Your SOUL is STOLEN by $n!", ch, NULL, victim, TO_VICT);
...
void spell_iron_skin( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  act(  "$n's skin turns to iron.", ch, NULL, NULL, TO_ROOM );
  af.type	 = sn;
  af.level	 = ch->level;
  af.duration	 = ch->level / 6;
  af.location	 = APPLY_AC;
...
void spell_chi_shield( int sn, int level, CHAR_DATA *ch, void *vo )
{
  ...
  if ( is_affected( ch, sn ) )
	return;
  send_to_char(  "You tap into your chi and use it to raise a chi shield.\n\r", ch );
  act(  "$n is surrounded by a chi shield.", ch, NULL, NULL, TO_ROOM );
  af.type	 = sn;
  af.level	 = ch->level * 2;
...
void spell_inspiration( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act(  "$n sings a song about great battles and brave heros.", 
	 ch, NULL, NULL, TO_ROOM );
	...
	{
	  if( victim == ch )
	    send_to_char(  "You have already been inspired.\n\r", victim );
	  else
	    act(  "$N has been inspired already.", ch, NULL, victim,
		 TO_CHAR );
/*	  return;  (still want to loop through rest of chars so just continue) */
	...
	if ( victim != ch)
	{
	  ...
	  send_to_char(  "You inspire yourself to greater heights.\n\r",
			ch );
    }
...
void spell_war_cry( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( saves_spell( level, victim ) )
    {
	act(  "$N ignores your cry for blood.",
	     ch, NULL, victim, TO_CHAR );
	return;
    }
    af.type	= sn;
	...
	send_to_char( 
		      ...
		      ch );
    send_to_char(  "A bloodthirsty cry sends shivers down your spine.\n\r",
		      victim );
...
void spell_group_healing( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char(  "You have completed your song.\n\r", ch );
    return;
...
void spell_chant( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "You begin your loud chant of destruction!\n\r", ch );
    act( "$n's chant wreaks havoc everywhere!", ch, NULL, NULL, TO_ROOM );
...
void spell_blur ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "Your form blurs.\n\r",
		 victim );
    act( "$N form becomes blurred, shifting and wavering before you.",
	ch, NULL, victim, TO_NOTVICT );
    return;
}
...
void spell_firewall( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (    IS_SET( ch->in_room->room_flags, ROOM_SAFE     )
	 ...
	 send_to_char(  "The spell fails to ignite.", ch );

    send_to_char( "A wall of fire explodes out in all direction from you!\n\r", ch );
    act( "A wall of fire explodes out from $n!", ch, NULL, NULL, TO_ROOM );
