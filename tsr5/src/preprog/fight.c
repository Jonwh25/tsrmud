...
/*            REMOVE_BIT(victim->act, PLR_THIEF);*/
	  ...
	  if ( !IS_NPC( victim ) )
	  {
		...
		{
		REMOVE_BIT( victim->act, PLR_THIEF );
		info( "%s the puny thief gets destroyed by the &rMERCENARY %s!",
		...
		info( "%s, the sly thief, has killed the &rMERCENARY %s.",
	...
	{
	  sprintf(log_buf, "%s &chas defeated %s &cin the arena!",
	          ch->name, victim->name);
	  wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0);
	  log_string(log_buf, CHANNEL_LOG, -1);
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    const  char         *attack;
	 ...
	 if ( dam ==   0 ) { vs = "miss";	vp = "misses";		}
			     ...
			     vp = "<<< ERADICATES >>>";			}
    else                   { vs = "do &rUNSPEAKABLE things to";
			     vp = "does &rUNSPEAKABLE things to";
}
    ...
    if ( dt == TYPE_HIT )
    {
	sprintf( buf1, "You &r%s $N%c",       vs, punct );
