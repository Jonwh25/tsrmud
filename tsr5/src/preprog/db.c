...
/* Add all new gsn's after 3/16/01 here */
int                     gsn_search;
int                     gsn_phase;
...
int                     port;
int                     control;                  /* for Copyover */
RECENT_DATA * recent_first;
RECENT_DATA * recent_x;
...
void	load_mobiles2   args( ( FILE *fp ) );
	...
	for ( ; ; )
	{
		...
		word = fread_word( fpArea );
		    ...
		    load_recall  ( fpArea );
	    	     ...
	    	     load_mobiles ( fpArea );
		else if ( !str_cmp( word, "OLDMOBILES"  ) )
	    	     load_mobiles2 ( fpArea );
     ...
     */
     {
       ...
       save_notes(); /* Load all boards */
       load_down_time( );

       fpArea = NULL;
       strcpy(strArea, "$");
       fBootDb = FALSE;
       daPrompt = str_dup( "{G<{m%h{ghp {m%m{gm {m%v{gmv{G>{x " );
  ...
  while( !done )
  {
    ...
    default:
      ...
      *traptypes |= pTrap->type;
      pTrap->arglist = fread_string( fp, &stat );
      fread_to_eol( fp );
      pTrap->disarmable = fread_number( fp );
      fread_to_eol( fp );
      pTrap->comlist = fread_string( fp, &stat );
      fread_to_eol( fp );
      num_trap_progs++;
...
/*	pHelp->area	= area_last ? area_last : NULL;	 not used Deck*/ /* OLC 1.1b */
	...
	pHelp->keyword	= fread_string( fp, &stat );
	if ( pHelp->keyword[0] == '$' )
	    break;
...
/*	char *spell;*/
	...
	pObjIndex->ac_spell             = fread_string( fp, &stat );
...
void load_rooms( FILE *fp )
{
	...
	pRoomIndex->room2_flags		= fread_number( fp );
	...
	for ( ; ; )
	{
		...
		pRoomIndex->owner = fread_string(fp, &stat );
	    }
	    else
	    {
...
void load_clans( void )
{
	...
	pClanIndex->name		= fread_string( fp, &stat );
	pClanIndex->diety               = fread_string( fp, &stat );
	pClanIndex->description         = fread_string( fp, &stat );
	pClanIndex->champ               = fread_string( fp, &stat );

	pClanIndex->leader              = fread_string( fp, &stat );
	pClanIndex->first               = fread_string( fp, &stat );
	pClanIndex->second              = fread_string( fp, &stat );
...
/*	fBootDb = TRUE; */
	...
	pSocialIndex->char_no_arg	=	fread_string( fp, &stat );
	pSocialIndex->others_no_arg	=	fread_string( fp, &stat );
	pSocialIndex->char_found	=	fread_string( fp, &stat );
	pSocialIndex->others_found	=	fread_string( fp, &stat );
	pSocialIndex->vict_found	=	fread_string( fp, &stat );
	pSocialIndex->char_auto		=	fread_string( fp, &stat );
	pSocialIndex->others_auto	=	fread_string( fp, &stat );
...
void load_down_time( void )
{
	...
	if ( !str_cmp( word, "DOWNTIME" ) )
	{
	    ...
	    down_time = fread_string( fp, &stat );
	}
	if ( !str_cmp( word, "WARNINGA" ) )
	{
	    free_string( warning1 );
	    warning1 = fread_string( fp, &stat );
	}
	if ( !str_cmp( word, "WARNINGB" ) )
	{
	    free_string( warning2 );
	    warning2 = fread_string( fp, &stat );
	}
    }
}
	...
	if ( pArea->nplayer > 0 && pArea->age == 15 - 1 )
	{
      		   ...
      		   )
		{
		    if ( pArea->reset_sound == NULL )
		      send_to_char( "You shiver as a cold breeze blows through the room.\n\r",
		      pch );
		    else if ( str_cmp( pArea->reset_sound, "none" ) )
                    {
		      send_to_char( pArea->reset_sound, pch );
	 	      send_to_char( "\n\r", pch );
  		    }
		}
	    }
...
void do_areas( CHAR_DATA *ch, char *argument )
{
  ...
  for ( pArea = area_first; pArea; pArea = pArea->next )
  {
    ...
    {
       sprintf( buf, "%-8.8s %-18.18s %-10.10s",
                pArea->name,
                &pArea->name[9],
                &pArea->name[28] ); 
       strcat( buf1, buf );
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level >= LEVEL_IMMORTAL )
    {
      ...
      sprintf( buf, "Mobs      %5d(%d)\n\r", top_mob_index, mobs_in_game ); send_to_char( buf, ch );
      sprintf( buf, "Objs      %5d\n\r", top_obj_index ); send_to_char( buf, ch );
      sprintf( buf, "Resets    %5d\n\r", top_reset     ); send_to_char( buf, ch );
      sprintf( buf, "Rooms     %5d\n\r", top_room      ); send_to_char( buf, ch );
      sprintf( buf, "Shops     %5d\n\r", top_shop      ); send_to_char( buf, ch );
      sprintf( buf, "MobProgs  %5d\n\r", num_mob_progs ); send_to_char( buf, ch );
      sprintf( buf, "TrapProgs %5d\n\r", num_trap_progs); send_to_char( buf, ch );

      sprintf( buf, "Strings %5d strings of %7d bytes (max %ld).\n\r",
...
void info( const char *str, int param1, int param2 )
{
  ...
  for ( d = descriptor_list; d; d = d->next )
  {
    ...
    {
      send_to_char("{GThe town fool yells, '",d->character);
...
void logch( char *l_str, int l_type, int lvl )
{
	...
	for ( d = descriptor_list; d; d = d->next )
	{
	  if ( d->connected != CON_PLAYING || IS_SET( d->character->deaf, l_type )
	    ...
	    continue;
		send_to_char(  log_str, d->character );
		send_to_char(  "\n\r", d->character );
	}
	return;
}
...
void load_newbie( void )
{
	...
	pNewbieIndex->answer1		=	fread_string( fp, &stat );
	pNewbieIndex->answer2		=	fread_string( fp, &stat );
	...
	for ( ; ; )
        {
            ...
            if ( !str_cmp( name, "#END"    ) )
            {
              ...
              pRace->race_full  = fread_string( fp, &stat );
            else if ( !str_cmp( name, "Race_Name" ) )
              pRace->race_name  = fread_string( fp, &stat );
...
/* Quzah's Recent stuff */
int recent_create( int size )
{
  RECENT_DATA *r = NULL;
  int count;
  ...
  {
    recent_first          = alloc_perm( sizeof( struct recent_data ) );
    recent_first->ptr     = recent_first;
    recent_first->name[0] = '\0';
    recent_first->logOn   = time(0);
    recent_first->logOff  = time(0);
    recent_first->used    = FALSE;
    recent_x              = recent_first;
  }


  for ( count = 0; count < size; count++ )
  {
    r          = alloc_perm( sizeof( struct recent_data ) );
    /* if ( r == NULL ) exit(1); */ /* Optional. */
    r->name[0] = '\0';
    r->logOn   = time(0);
    r->logOff  = time(0);
    r->used    = FALSE;
    r->ptr     = recent_first->ptr;
...
int recent_add( CHAR_DATA *ch ) {
  struct recent_data* r = NULL;
  ...
  if ( recent_x == NULL || recent_first == NULL )
  {
    if ( recent_create( 20 ) < 0 )
    {
  ...
  /* Skip through the list until we find the one that is 1 off from x. */
  for ( r = recent_x->ptr; r->ptr != recent_x; r = r->ptr ) ;


  sprintf( r->name, ch->name );
  r->logOn  = ch->logon;
  r->logOff = time(0);
  r->used   = TRUE;
  recent_x  = recent_x->ptr;
