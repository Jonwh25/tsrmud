...
/* XORPHOX perm spells */
	...
	if (fAdd) {
		...
		act(skill_table[sn].room_msg_off, ch, NULL, NULL, TO_ROOM);
	}
	break;
    case APPLY_SNEAK:
	sn = gsn_sneak;
	if (fAdd) {
	    ...
	    affect_to_char(ch, &af);
	    send_to_char("You move silently.\n\r", ch);
 ...
 */
	...
	/* if(fAdd)
	   ...
	   act( "The flames about $n's body steam away to nothing.",
	   ch, NULL, NULL, TO_ROOM );
	   }
	   } */
...
/* END */
	...
	if (fAdd && !is_affected(ch, psn)) {
		...
		send_to_char(skill_table[psn].msg_off, ch);
		send_to_char("\n\r", ch);
		if (skill_table[psn].room_msg_off) {
		    act(skill_table[psn].room_msg_off, ch, NULL, NULL, TO_ROOM);
		}
	    }
	}
    }
...
bool can_see_obj(CHAR_DATA * ch, OBJ_DATA * obj)
{
    ...
    if (IS_SET(obj->extra_flags, ITEM_LOCKER))
	return FALSE;
...
bool can_drop_obj(CHAR_DATA * ch, OBJ_DATA * obj)
{
    ...
    if (!IS_NPC(ch) && ch->level >= LEVEL_IMMORTAL &&
	!IS_SET(obj->extra_flags, ITEM_LOCKER))
	return TRUE;
...
bool is_room_owner(CHAR_DATA * ch, ROOM_INDEX_DATA * room)
{
    ...
    return is_name(ch, ch->name, room->owner);
}

/* Quzah's Recent Stuff */
void timeString(time_t time, char here[])
{
    int count;
