...
void tedit(CHAR_DATA * ch, char *argument)
{
    ...
    case ED_RPROG:
	{
		...
		if (trap == (TRAP_DATA *) ch->desc->pEdit)
		    break;
	    if (!trap) {
		send_to_char("Room changed.  Returning to REditor.\n\r", ch);
		edit_done(ch);
/*      redit( ch, arg ); */
		return;
	    }
...
/*      redit(ch, arg); */
    ...
    case ED_EPROG:
	if (!pExit) {
	    send_to_char("Exit not found.  Returning to REditor.\n\r", ch);
	...
	if ((value = flag_value(eprog_types, arg)) != NO_FLAG) {
	    ...
	    SET_BIT(pArea->area_flags, AREA_CHANGED);
	    send_to_char("ExitProg type set.\n\r", ch);
	    return;
	}
	break;
    default:
...
bool redit_epedit(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_prefix(command, dir_name[dir]) && ch->in_room->exit[dir])
	    break;
    if (dir == 6) {
	send_to_char("EPEdit:  Room has no such exit.\n\r", ch);
	return FALSE;
    }
    argument = one_argument(argument, command);
