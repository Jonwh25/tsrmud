...
bool redit_proglist(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0') {
	send_to_char("Syntax:  proglist <all/obj/mob/room>\n\r", ch);
	return FALSE;
    }
	...
	found = TRUE;
	send_to_char("Objects with programs:\n\r", ch);
	...
	found = TRUE;
	send_to_char("Rooms with programs:\n\r", ch);
	for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++) {
	...
	found = TRUE;
	send_to_char("Mobs with programs:\n\r", ch);
	for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++) {
    ...
    if (!found) {
	if (!str_cmp(arg, "all")) {
	    send_to_char("Programs not found in this area.\n\r", ch);
	    return FALSE;
	} else {
...
bool redit_olist(CHAR_DATA * ch, char *argument)
{
    ...
    one_argument(argument, arg);
    if (arg[0] == '\0') {
	send_to_char("Syntax:  olist <all/name/item_type/object level>\n\r", ch);
	return FALSE;
    }
    pArea = ch->in_room->area;
    buf1[0] = '\0';
...
bool aedit_show(CHAR_DATA * ch, char *argument)
{
    ...
    EDIT_AREA(ch, pArea);
    sprintf(buf, "                  01234567 0123456789012345678 0123456789\n\r");
    send_to_char(buf, ch);
    ...
    sprintf(buf, "Sounds:   %s\n\r",
       ...
       "(default: You shiver as a cold breeze blows through the room.)");
    send_to_char(buf, ch);
    ...
    if (!stat(buf, &statis)) {
	sprintf(buf, "Last modifed on Build: %s\r", (char *) ctime(&statis.st_mtime));
	send_to_char(buf, ch);
    } else
	send_to_char("Last modifed on Build: (No such area)\n\r", ch);

    sprintf(buf, "../../EOS/area/%s", pArea->filename);
    if (!stat(buf, &statis)) {
	sprintf(buf, "Last modifed on Storm: %s\r", (char *) ctime(&statis.st_mtime));
	send_to_char(buf, ch);
    } else
	send_to_char("Last modifed on Storm: (No such area)\n\r", ch);
...
bool aedit_links(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("Rooms that have exits into this area:\n\r", ch);
    ...
    send_to_char("Rooms that have exits leaving this area:\n\r", ch);
...
bool aedit_llevel(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  llevel [#lowest sugested level]\n\r", ch);
	return FALSE;
    }
    send_to_char("Lowest sugested level set.\n\r", ch);
...
bool aedit_ulevel(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  ulevel [#upper most sugested level]\n\r", ch);
	return FALSE;
    }
    pArea->ulevel = atoi(ulevel);

    send_to_char("Upper most suggested level set.\n\r", ch);
    return TRUE;
}
...
bool aedit_sounds(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax:   sounds [$reset_sounds]\n\r", ch);
	return FALSE;
    }
...
bool aedit_clan_hq(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("You are *too* low of trust to do this.\n\r", ch);
	return FALSE;
    }
    return TRUE;
}
...
bool redit_show(CHAR_DATA * ch, char *argument)
{
    ...
    if (pRoom->rd > 0) {
	sprintf(buf, "Room damage amount: [%d]\n\r", pRoom->rd);
	strcat(buf1, buf);
    }
...
bool redit_rdamage(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax: rdamage [ammount]\n\r", ch);
	return FALSE;
    }
    argument = one_argument(argument, arg);
    if (!is_number(arg)) {
	send_to_char("Amount must be the number of hp the room is to damage the CH per update.\n\r", ch);
	return FALSE;
    }
    damage = atoi(arg);
    pRoom->rd = damage;
...
{
    send_to_char("If you want to DELETE the room, spell it out.\n\r", ch);
    return FALSE;
}
...
/*      newobj = create_object( pObjIndex, number_fuzzy( olevel ) );  */
	...
	if (!pMob) {
	    send_to_char("Mobile not reset in this room.\n\r", ch);
	    return FALSE;
	}
	...
	if (!IS_SET(pObjIndex->wear_flags, ITEM_TAKE)) {
	    send_to_char("REdit:  Object needs take flag.\n\r", ch);
	    return FALSE;
	}
	/*
...
bool redit_rreset(CHAR_DATA * ch, char *argument)
{
    ...
    one_argument(argument, arg);
    if (arg[0] == '\0') {
	send_to_char("Syntax: rreset <last-door>\n\r", ch);
	return FALSE;
    }
    if (is_number(arg))
	direc = atoi(arg);
...
void show_obj_values(CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
    ...
    case ITEM_PORTAL:
	sprintf(buf, "[v0] Destination: [vnum %d]", obj->value[0]);
	send_to_char(buf, ch);
	break;
    case ITEM_WAND:
    case ITEM_LENSE:
	...
	sprintf(buf,
		"[v0] Decoder item:        [%d] [%s]\n\r"
		"[v1] Minimum read level:  [%d]\n\r"
		"[v2] Minimum write level: [%d]\n\r",
		obj->value[0],
...
bool set_ac_type(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  ac_type [type #]\n\r", ch);
	send_to_char("         type #1 = oload item.\n\r", ch);
	send_to_char("              #2 = mload mob.\n\r", ch);
	send_to_char("              #3 = transfer character.\n\r", ch);
	send_to_char("              #4 = item morph.\n\r", ch);
	send_to_char("              #5 = item cast spell.\n\r", ch);
	return FALSE;
    }
...
bool set_ac_vnum(CHAR_DATA * ch, char *argument)
{
	...
	return FALSE;
    case 0:
	send_to_char("Obj ac_type does not require a vnum.\n\r", ch);
	return FALSE;
    case 1:
...
bool set_ac_v1(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  ac_v1 [ current charges ]\n\r", ch);
	return FALSE;
    }
...
bool set_ac_v2(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  ac_v2 [ max charges (-1 unlimited) ]\n\r", ch);
	return FALSE;
    }
    value = atoi(argument);
    if (value < -1)
...
bool set_ac_setspell(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  ac_setspell [ valid spell name ]\n\r", ch);
	return FALSE;
    }
...
bool set_obj_values(CHAR_DATA * ch, OBJ_INDEX_DATA * pObj, int value_num, char *argument)
{
	...
	default:
	    ...
	    return FALSE;
	case 0:
	    send_to_char("DECODER VNUM SET.\n\r\n\r", ch);
	    pObj->value[0] = atoi(argument);
	    break;
	case 1:
	    if (atoi(argument) > get_trust(ch)) {
		send_to_char("Limited by your trust.\n\r", ch);
		return FALSE;
	    }
	    send_to_char("MINIMUM READ LEVEL SET.\n\r\n\r", ch);
	    pObj->value[1] = atoi(argument);
	    break;
	case 2:
	    if (atoi(argument) > get_trust(ch)) {
	    ...
	    send_to_char("MINIMUM WRITE LEVEL SET.\n\r\n\r", ch);
	    pObj->value[2] = atoi(argument);
	    break;
	}
	break;
	...
	default:
	    ...
	    return FALSE;
	case 0:
	    send_to_char("MAXIMUM AMOUT OF BLOOD SET.\n\r\n\r", ch);
	    pObj->value[0] = atoi(argument);
	    break;
	case 1:
	    send_to_char("CURRENT AMOUNT OF BLOOD SET.\n\r\n\r", ch);
	    pObj->value[1] = atoi(argument);
	    break;
	}
	break;
...
bool oedit_show(CHAR_DATA * ch, char *argument)
{
	...
	sprintf(buf, "Joins to create: [%d] "
		"[%s]\n\r", pObj->join,
	    pJoinObj ? pJoinObj->short_descr : "Error: No such object.");
	send_to_char(buf, ch);
    }
    if (pObj->sep_one) {
	pJoinObj = get_obj_index(pObj->sep_one);
	sprintf(buf, "First seperated vnum is: [%d] "
		"[%s]\n\r", pObj->sep_one,
	    pJoinObj ? pJoinObj->short_descr : "Error: No such object.");
	send_to_char(buf, ch);
    }
	...
	sprintf(buf, "Second seperated vnum is: [%d] "
		"[%s]\n\r", pObj->sep_two,
	    pJoinObj ? pJoinObj->short_descr : "Error: No such object.");
	send_to_char(buf, ch);
    }
    sprintf(buf, "Invoke Type: [%d]  "
	    " Invoke Vnum: [%d]\n\r",
	    pObj->ac_type, pObj->ac_vnum);
    send_to_char(buf, ch);
    if (pObj->ac_charge[1] == -1)
	sprintf(buf, "Invoke is:   PERMANENT\n\r");
    else
	sprintf(buf, "Invoke charges:[%d/%d]\n\r",
		pObj->ac_charge[0], pObj->ac_charge[1]);
    send_to_char(buf, ch);
    if ((pObj->ac_type == 5) && (pObj->ac_spell != '\0'))
	sprintf(buf, "Invoke Spell:[%s]\n\r",
		pObj->ac_spell);
    else
	sprintf(buf, "Invoke Spell:[!NONE!]\n\r");
    send_to_char(buf, ch);
    if (pObj->extra_descr) {
	EXTRA_DESCR_DATA *ed;
...
bool oedit_join(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0' || !is_number(arg)) {
	send_to_char(" Syntax: ojoin [vnum]\n\r", ch);
	return FALSE;
    }
    value = atoi(arg);
...
bool oedit_sepone(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0' || !is_number(arg)) {
	send_to_char(" Syntax:  osepone [vnum]\n\r", ch);
	return FALSE;
    }
    value = atoi(arg);
    if (value < 0 || value > 33000) {
...
bool oedit_septwo(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0' || !is_number(arg)) {
	send_to_char(" Syntax: oseptwo [vnum]\n\r", ch);
	return FALSE;
    }
    value = atoi(arg);
    if (value < 0 || value > 33000) {
...
bool oedit_cost(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_cmp(arg, "gold")) {
	    ...
	    send_to_char("Silver cost set.\n\r", ch);
	} else if (!str_cmp(arg, "copper")) {
	    pObj->cost.copper = atoi(arg2);
	    send_to_char("Copper cost set.\n\r", ch);
	} else {
...
{
    send_to_char("If you want to DELETE this object, spell it out.\n\r", ch);
    return FALSE;
}
...
/*    sprintf( buf,
   ...
   pClan->obj_vnum_1, pClan->obj_vnum_2, pClan->obj_vnum_3  ); */
	...
	if ((pObjIndex = get_obj_index(pClan->obj_vnum_1)))
	    sprintf(buf, "Level 50:  [%5d] %s\n\r",
		    pClan->obj_vnum_1, pObjIndex->short_descr);
	else
	    sprintf(buf, "Level 50:  [%5d] No such object!!\n\r",
		    pClan->obj_vnum_1);
	send_to_char(buf, ch);
	if ((pObjIndex = get_obj_index(pClan->obj_vnum_2)))
	    sprintf(buf, "Level 75:  [%5d] %s\n\r",
		    pClan->obj_vnum_2, pObjIndex->short_descr);
	else
	    sprintf(buf, "Level 75:  [%5d] No such object!!\n\r",
		    pClan->obj_vnum_1);
	send_to_char(buf, ch);
	if ((pObjIndex = get_obj_index(pClan->obj_vnum_3)))
	    sprintf(buf, "Level 100: [%5d] %s\n\r",
		    pClan->obj_vnum_3, pObjIndex->short_descr);
	else
	    sprintf(buf, "Level 100: [%5d] No such object!!\n\r",
		    pClan->obj_vnum_1);
	send_to_char(buf, ch);
    } else {
	if ((pObjIndex = get_obj_index(pClan->obj_vnum_1)))
	    sprintf(buf, "Level 30:  [%5d] %s\n\r",
		    pClan->obj_vnum_1, pObjIndex->short_descr);
	else
	    sprintf(buf, "Level 30:  [%5d] No such object!!\n\r",
		    pClan->obj_vnum_1);
	send_to_char(buf, ch);
	if ((pObjIndex = get_obj_index(pClan->obj_vnum_2)))
	    sprintf(buf, "Level 65:  [%5d] %s\n\r",
		    pClan->obj_vnum_2, pObjIndex->short_descr);
	else
	    sprintf(buf, "Level 65:  [%5d] No such object!!\n\r",
		    pClan->obj_vnum_1);
	send_to_char(buf, ch);
	if ((pObjIndex = get_obj_index(pClan->obj_vnum_3)))
	    sprintf(buf, "Level 100: [%5d] %s\n\r",
...
bool cedit_pkilled(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax: pkilled [number]\n\r", ch);
	return FALSE;
    }
    pClan->pdeaths = atoi(argument);

    send_to_char("PKilled set.\n\r", ch);
    return TRUE;
}
...
bool cedit_mkills(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax: mkills [number]\n\r", ch);
	return FALSE;
    }
    pClan->mkills = atoi(argument);

    send_to_char("Mkills set.\n\r", ch);
    return TRUE;
}
...
bool cedit_diety(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax:  deity [string]\n\r", ch);
	return FALSE;
    }
...
bool cedit_civil(CHAR_DATA * ch, char *argument)
{
    ...
    if IS_SET
	(pClan->settings, CLAN_CIVIL_PKILL)
	    send_to_char("Clan switched to Civil Pkill.\n\r", ch);
    else
	send_to_char("Clan switched to No Civil Pkill.\n\r", ch);
...
bool cedit_pkill(CHAR_DATA * ch, char *argument)
{
    ...
    if IS_SET
	(pClan->settings, CLAN_PKILL)
	    send_to_char("Clan switched to Pkill.\n\r", ch);
    else
	send_to_char("Clan switched to Peace.\n\r", ch);
...
bool cedit_object(CHAR_DATA * ch, char *argument)
{
    ...
    if (number[0] == '\0') {
	send_to_char("Syntax: object <1/2/3> <vnum>\n\r", ch);
	return FALSE;
    }
    value = atoi(number);
    vnum = atoi(onum);
...
bool cedit_power(CHAR_DATA * ch, char *argument)
{
    ...
    if (number[0] == '\0') {
	send_to_char("Syntax: power <1/2/3/4> <name>\n\r", ch);
	...
	send_to_char("       4 -> Champion of Clan.\n\r", ch);
	send_to_char("To clear a position, in the <name> field type: none", ch);
	return FALSE;
    }
    value = atoi(number);
...
bool cedit_induct(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Sets which clan position can induct.", ch);
	send_to_char("Syntax: induct <1/2/3/4>\n\r", ch);
	send_to_char("Valid positions are as follows\n\r", ch);
	send_to_char("       1 -> Centurion of Clan.\n\r", ch);
	send_to_char("       2 -> Council of Clan.\n\r", ch);
	send_to_char("       3 -> Leader of Clan.\n\r", ch);
	send_to_char("       4 -> Champion of Clan.\n\r", ch);
...
bool medit_create(CHAR_DATA * ch, char *argument)
{
	...
	if (value > pArea->uvnum) {
	    send_to_char("MEdit:  No free mob vnums in this area.\n\r", ch);
	    return FALSE;
	}
    }
    /* OLC 1.1b */
...
{
    send_to_char("If you want to DELETE this mob, spell it out.\n\r", ch);
    return FALSE;
}
...
bool medit_class(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax: class [mobs class]\n\r", ch);
	return FALSE;
    }
    ...
    send_to_char("Incorrect class number.\n\r", ch);
    return FALSE;
}
...
bool medit_gold(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  gold [amount]\n\r", ch);
	return FALSE;
    }
#ifdef NEW_MONEY
...
bool medit_silver(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  silver [amount]\n\r", ch);
	return FALSE;
    }
    pMob->money.silver = atoi(argument);

    send_to_char("Silver coins set.\n\r", ch);
    return TRUE;
}
...
bool medit_copper(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  copper [amount]\n\r", ch);
	return FALSE;
    }
    pMob->money.copper = atoi(argument);

    send_to_char("Copper coins set.\n\r", ch);
    return TRUE;
}
#endif
...
bool medit_shop(CHAR_DATA * ch, char *argument)
{
	...
	if (!pMob->pShop) {
	    send_to_char("REdit:  No shop to remove.\n\r", ch);
	    return FALSE;
	}
...
bool redit_eplist(CHAR_DATA * ch, char *argument)
{
	...
	if (!str_prefix(argument, dir_name[dir]) &&
	    ...
	    break;
    if (dir == 6) {
	send_to_char("Exit does not exist in this room.\n\r", ch);
	return FALSE;
    }
...
bool redit_epremove(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  epremove <direction> #\n\r", ch);
	return FALSE;
    }
    vnum = atoi(argument);
...
bool tedit_disarmable(CHAR_DATA * ch, char *argument)
{
    ...
    if (pTrap->disarmable)
	send_to_char("Trap is now disarmable.\n\r", ch);
    else
	send_to_char("Trap is no longer disarmable.\n\r", ch);
    return TRUE;
}
...
bool sedit_show(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char(buf, ch);
    sprintf(buf,
	    "(1) Char_no_arg:    <social>          You see.\n\r%s\n\r",
	    pSocial->char_no_arg);
    send_to_char(buf, ch);
    sprintf(buf,
	    "(2) Others_no_arg:  <social>          Room see.\n\r%s\n\r",
	    pSocial->others_no_arg);
    send_to_char(buf, ch);
    sprintf(buf,
	    "(3) Char_found:     <social> <victim> You see.\n\r%s\n\r",
	    pSocial->char_found);
    send_to_char(buf, ch);
    sprintf(buf,
	    "(4) Others_found:   <social> <victim> Room see.\n\r%s\n\r",
	    pSocial->others_found);
    send_to_char(buf, ch);
    sprintf(buf,
	    "(5) Victim_found:   <social> <victim> Victim see.\n\r%s\n\r",
	    pSocial->vict_found);
    send_to_char(buf, ch);
    sprintf(buf,
	    "(6) Char_auto:      <social> self     You see.\n\r%s\n\r",
	    pSocial->char_auto);
    send_to_char(buf, ch);
    sprintf(buf,
	    "(7) Others_auto:    <social> self     Room see\n\r%s\n\r",
	    pSocial->others_auto);
    send_to_char(buf, ch);
...
bool rename_show(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("REMAKE AN ITEM: Type done when finished.\n\r\n\r", ch);
    sprintf(buf, "1) Keyword(s):        Syntax: 1 <new keyword>\n\r[%s]\n\r", pObj->name);
    send_to_char(buf, ch);
    sprintf(buf, "2) Short:             Syntax: 2 <Looks in inv or equiped.>\n\r[%s]\n\r",
	    pObj->short_descr);
    send_to_char(buf, ch);
    sprintf(buf, "3) Long:              Syntax: 3 <Looks when on the ground or looked at.>\n\r[%s]\n\r",
	    pObj->description);
    send_to_char(buf, ch);
...
bool forge_show(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("Forging an item; type DONE only when finished.\n\r", ch);
    send_to_char("Name and descriptions:\n\r", ch);
    ...
    send_to_char("Availble stats and affects:\n\r", ch);
    sprintf(buf, "[Stat 1:       +%d] [Damroll:      +%d] [Hitroll:       +%d]\n\r",
	    max_stat, max_dam, max_hit);
    send_to_char(buf, ch);
    sprintf(buf, "[Hit Points: +%d] [Mana:        +%d] [Armor Class:  %d]",
	    max_hp, max_mana, max_ac);
    send_to_char(buf, ch);
    sprintf(buf, "\n\r[Blood:       +%d] ", max_bp);
    send_to_char(buf, ch);
    if (pObj->level >= 40) {
	sprintf(buf, "[Saving-Spell: %d] [Saving-Breath: %d]\n\r",
		max_saves, max_saveb);
	send_to_char(buf, ch);
    }
    if (pObj->level >= 45) {
	sprintf(buf, "[Stat 2:        +%d]", max_stat);
	send_to_char(buf, ch);
    }
    if (pObj->level >= 60) {
	sprintf(buf, " [Anti-Disarm: +%d]", max_ad);
	send_to_char(buf, ch);
    }
    if (pObj->level >= 101) {
	sprintf(buf, " [Stat 3:        +%d]", max_stat);
	send_to_char(buf, ch);
    }
    send_to_char("\n\r", ch);
    send_to_char("Added stats and affects:\n\r", ch);
    send_to_char("#- [  affect  ] [modifier]\n\r", ch);
    for (cnt = 1, paf = pObj->affected; cnt <= pObj->level / 10; cnt++, paf = paf_next) {
	if (cnt == 7)
	    break;
	if (paf) {
	    paf_next = paf->next;
	    sprintf(buf, "%d- [%10s] [%8d]\n\r", cnt,
		    flag_string(apply_flags, paf->location),
		    paf->modifier);
	} else
	    sprintf(buf, "%d- [      none] [       0]\n\r", cnt);
	send_to_char(buf, ch);
    }
#ifdef NEW_MONEY
    sprintf(buf, "Gold Cost:   [%d]\n\r"
	    "Silver Cost: [%d]\n\r"
	    "Copper Cost: [%d]\n\r",
	    pObj->cost.gold, pObj->cost.silver, pObj->cost.copper);
#else
    sprintf(buf, "Object Cost: [%d]\n\r", pObj->cost);
#endif
    send_to_char(buf, ch);
    return FALSE;
}
bool forge_addaffect(CHAR_DATA * ch, char *argument)
{
    ...
    if (loc[0] == '\0' || mod[0] == '\0' || !is_number(mod)) {
	send_to_char("Syntax: # [affect] [modifier]\n\r", ch);
	return FALSE;
    }
	...
	if (cnt >= pObj->level / 10 || cnt >= 6) {
	    send_to_char("You can no longer add anything to this item.\n\r", ch);
	    return FALSE;
	}
	...
	if (stat_cnt >= max_statn
	    ...
	    send_to_char("You have already added the maximum number of stats possible for your experience.\n\r", ch);
	    return FALSE;
	}
    }
    pAf = new_affect();
...
bool forge_type(CHAR_DATA * ch, char *argument)
{
    ...
    FORGE_OBJ(ch, pObj);
    if (pObj->item_type != ITEM_WEAPON) {
	send_to_char("You are not forging a weapon.\n\r", ch);
	return FALSE;
    }
    if (argument[0] == '\0') {
...
void do_opstat(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0') {
	send_to_char("ObjProg stat what?\n\r", ch);
	return;
    }
...
bool mreset_show(CHAR_DATA * ch, char *argument)
{
    ...
    sprintf(buf, "Mobile Reset: [%d] %s\n\r", pRoom->vnum, pRoom->name);
    send_to_char(buf, ch);
    send_to_char("[ #] [MVnum] [Num] Mob Short Description\n\r", ch);

    for (pReset = pRoom->reset_first; pReset; pReset = pReset->next) {
	if (pReset->command == 'M') {
	    pMobIndex = get_mob_index(pReset->arg1);
	    sprintf(buf, "[%2d] [%5d] [%3d] %s\n\r",
		    count++, pReset->arg1, pReset->arg2,
	    pMobIndex ? pMobIndex->short_descr : "Mob does not exist!!");
	    send_to_char(buf, ch);
	}
    }
    return FALSE;
...
bool spedit_dispelable(CHAR_DATA * ch, char *argument)
{
    ...
    if (pSpell->dispelable)
	send_to_char("Spell switched to dispelable.", ch);
    else
	send_to_char("Spell switched to non dispelable.", ch);
...
bool spedit_damage_msg(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("Spell damage message set.\n\r", ch);
    return TRUE;
...
bool spedit_spell_ends(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("Spell wear off message set.\n\r", ch);
    return TRUE;
...
bool spedit_min(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  min [class] [level]\n\r", ch);
	return FALSE;
    }
    if (strlen(class) > 3)
	lng = TRUE;
...
bool nedit_answer1(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax: answer1 [string]\n\r", ch);
	return FALSE;
    }
    free_string(pNewbie->answer1);
    pNewbie->answer1 = str_dup(argument);
    send_to_char("First answer set.\n\r", ch);
...
bool nedit_answer2(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax: answer2 [string]\n\r", ch);
	return FALSE;
    }
    free_string(pNewbie->answer2);
    pNewbie->answer2 = str_dup(argument);
    send_to_char("Second answer set.\n\r", ch);
...
bool race_edit_mstr(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  mstr [number]\n\r", ch);
	return FALSE;
    }
    pRace->mstr = atoi(argument);

    send_to_char("MStr set.\n\r", ch);
    return TRUE;
}
...
bool race_edit_mint(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  mint [number]\n\r", ch);
	return FALSE;
    }
    pRace->mint = atoi(argument);

    send_to_char("MInt set.\n\r", ch);
    return TRUE;
}
...
bool race_edit_mwis(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  mwis [number]\n\r", ch);
	return FALSE;
    }
    pRace->mwis = atoi(argument);

    send_to_char("MWis set.\n\r", ch);
    return TRUE;
}
...
bool race_edit_mdex(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  mdex [number]\n\r", ch);
	return FALSE;
    }
    pRace->mdex = atoi(argument);

    send_to_char("MDex set.\n\r", ch);
    return TRUE;
}
...
bool race_edit_mcon(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0' || !is_number(argument)) {
	send_to_char("Syntax:  mcon [number]\n\r", ch);
	return FALSE;
    }
    pRace->mcon = atoi(argument);

    send_to_char("MCon set.\n\r", ch);
    return TRUE;
}
...
bool race_edit_name(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  name [string] (string has to be 3 characters long)\n\r", ch);
	return FALSE;
    }
...
bool race_edit_full(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Syntax:  full [string] (String can't be more than 20 characters long.)\n\r", ch);
	return FALSE;
    }
