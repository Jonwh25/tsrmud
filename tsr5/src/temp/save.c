...
bool load_char_obj(DESCRIPTOR_DATA * d, char *name)
{
    ...
    *ch->pcdata = pcdata_zero;
    ...
    /* every characters starts at default board 
     * from login.. this board should be read_level ==0!
     */   
...
void fread_char(CHAR_DATA * ch, FILE * fp)
{
	...
	case 'A':
	    ...
	    KEY("Afkmes", ch->pcdata->afkchar, fread_string(fp, &stat));
		...
		pAl->old = fread_string(fp, &stat);
		pAl->new = fread_string(fp, &stat);
	...
	case 'B':
	    KEY("Bmfin", ch->pcdata->bamfin, fread_string(fp, &stat));
	    KEY("Bmfout", ch->pcdata->bamfout, fread_string(fp, &stat));
	    KEY("Bmfsee", ch->pcdata->bamfusee, fread_string(fp, &stat));
#ifdef NEW_MONEY
	...
	case 'D':
	    ...
	    KEY("Detract", ch->pcdata->detractments, fread_string(fp, &stat));
	    KEY("Dscr", ch->description, fread_string(fp, &stat));
	    break;

	case 'E':
	    KEY("Email", ch->pcdata->email, fread_string(fp, &stat));
	    KEY("Empower", ch->pcdata->empowerments, fread_string(fp, &stat));
	    if (!str_cmp(word, "End")) {
...
/*              if ( ch->trust > 107 && ch->level < 108 && !IS_CODER(ch) )
   ch->trust = 0; */
	...
	case 'H':
	    ...
	    KEY("Host", ch->pcdata->host, fread_string(fp, &stat));
	...
	case 'I':
	    ...
	    KEY("Ignore",  ch->pcdata->ignore, fread_string(fp, &stat));
	    break;
	case 'L':
	    KEY("Lnm", ch->pcdata->lname, fread_string(fp, &stat));
	...
	case 'P':
	    ...
	    KEY("Paswd", ch->pcdata->pwd, fread_string(fp, &stat));
	    KEY("Playd", ch->played, fread_number(fp));
	    KEY("Plan", ch->pcdata->plan, fread_string(fp, &stat));
	    KEY("Pos", ch->position, fread_number(fp));
	    KEY("Prac", ch->practice, fread_number(fp));
	    KEY("Prmpt", ch->prompt, fread_string(fp, &stat));
	    break;
...
#endif
	    KEY("Slyuc", ch->pcdata->slayusee, fread_string(fp, &stat));
	    KEY("Slyrm", ch->pcdata->slayroom, fread_string(fp, &stat));
	    KEY("Slyvict", ch->pcdata->slayvict, fread_string(fp, &stat));
	...
	case 'T':
	    ...
	    KEY("Trnsto", ch->pcdata->transto, fread_string(fp, &stat));
	    KEY("Trnsfrom", ch->pcdata->transfrom, fread_string(fp, &stat));
	    KEY("Trnvict", ch->pcdata->transvict, fread_string(fp, &stat));

	    if (!str_cmp(word, "Ttle")) {
		ch->pcdata->title = fread_string(fp, &stat);
	...
	case 'W':
	    ...
	    KEY("Whotype", ch->pcdata->whotype, fread_string(fp, &stat));
	    break;
	}
...
void fread_obj(CHAR_DATA * ch, FILE * fp, bool storage)
{
    ...
    *obj = obj_zero;
	...
	case 'A':
		...
		obj->ac_spell = fread_string(fp, &stat);
		fMatch = TRUE;
		break;
	    }
	...
	case 'D':
	    KEY("Description", obj->description, fread_string(fp, &stat));
	    break;
...
/*                  obj->next = obj_free;
   obj_free  = obj; */
	...
	case 'N':
	    KEY("Name", obj->name, fread_string(fp, &stat));
	...
	case 'S':
	    KEY("ShortDescr", obj->short_descr, fread_string(fp, &stat));
#ifdef NEW_MONEY
...
void save_pet(CHAR_DATA * ch, FILE * fp, CHAR_DATA * pet)
{
    AFFECT_DATA *paf;
    fprintf(fp, "#PET\n");
    fprintf(fp, "%d %d %d %d %d %d %ld %ld %d %s~ %s~\n", pet->pIndexData->vnum,
	    pet->hit, pet->perm_hit, pet->mod_hit, pet->act, pet->act2, pet->affected_by, pet->affected_by2,
...
void fread_pet(CHAR_DATA * ch, FILE * fp)
{
    ...
    pet->short_descr = str_dup(fread_string(fp, &stat));
    free_string(pet->long_descr);
    pet->long_descr = str_dup(fread_string(fp, &stat));
...
void fwrite_finger(CHAR_DATA * ch, FILE * fp)
{
    ...
    fprintf(fp, "Name: %s\n", ch->name);
    fprintf(fp, "Mud Age: %-13d", get_age(ch));
    fprintf(fp, "Clan: %s\n", get_clan_index(ch->clan)->name);
    fprintf(fp, "Level: %-16d", ch->level);
    fprintf(fp, "Sex: %s\n", ch->sex == SEX_MALE ? "male" :
	    ch->sex == SEX_FEMALE ? "female" : "neutral");
    fprintf(fp, "Class: %-15s", class_short(ch));
