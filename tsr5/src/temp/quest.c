...
/* Quest area: info on area only for object quests */
	...
	if (IS_SET(ch->act, PLR_QUESTOR)) {
		...
		send_to_char("You aren't currently on an object quest.\n\r", ch);
	    return;
	} else
	    send_to_char("You aren't currently on a quest.\n\r", ch);
	return;
...
/* List displaying items one can buy with quest points */
	...
	if (arg2[0] == '\0') {
	    send_to_char("To buy an item, type 'QUEST BUY <item> <amount>'.\n\rAmount is optional.\n\r", ch);
	    return;
	}
	for (cnt = 0; quest_table[cnt].name[0] != '\0'; cnt++) {
		...
		if (ch->questpoints >= quest_table[cnt].qp * amt) {
			...
			if (is_name(ch, arg2, "prac pracs practice practices")) {
			    ch->practice += amt;
			    if (amt > 1) {
				act("$N gives some practice sessions to $n.", ch, NULL, questman, TO_ROOM);
				act("$N gives you some practice sessions.", ch, NULL, questman, TO_CHAR);
			    } else {
				act("$N gives a practice session to $n.", ch, NULL, questman, TO_ROOM);
				act("$N gives you a practice session.", ch, NULL, questman, TO_CHAR);
			    }
			} else {
			    while (amt > 0) {
...
/* Quest not complete, but still time left */
	...
	send_to_char("QUEST commands: POINTS INFO TIME REQUEST COMPLETE LIST BUY AREA.\n\r", ch);
...
/* Player has a quest, turn all channels off except gossip and yell */
