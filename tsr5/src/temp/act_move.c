...
/* void move_char( CHAR_DATA *ch, int door ) */
{
    ...
    if ( ( IS_AFFECTED(ch, AFF_ANTI_FLEE) ) && ( !Fall ) )
	 ...
	 send_to_char( "You are too frightened to move.\n\r", ch );
       else
    ...
    /* For Rented Rooms */
    if (IS_SET(to_room->room2_flags, ROOM_RENTED))
    {
       if (!IS_IMMORTAL(ch))
       {
	...
	{
	send_to_char(  "That room is for dwarves only.\n\r", ch );
	return;
	}
    if ( !IS_NPC( ch ) )
    {
...
/* Blood trickles from your wounds.\n\r */
	       ...
	       send_to_char(  "Blood trickles from your wounds.\n\r", ch );
	       act(  "$n leaves a trail of blood.", ch , NULL, NULL, TO_ROOM );
	...
	&& ( ch->race != RACE_HALFLING ) && ( !Fall ) )
          act( "$n arrives from $T.", ch, NULL,
	      dir_noun[rev_dir[door]], TO_ROOM );

    do_look( ch, "auto" );

    if ( Fall )
        act(  "$n falls down from above.", ch, NULL, NULL, TO_ROOM ); 
    ...
    {
      act(  "$n falls down to the room below.\n\r", ch,
       NULL, NULL, TO_ROOM );
      act(  "You fall through where the floor should have been!\n\r",
...
void do_rest( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "You wake up and start resting.\n\r", ch );
  	if ( !IS_AFFECTED(ch,AFF_HIDE) )
	  act( "$n wakes up and rests.", ch, NULL, NULL, TO_ROOM );
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    {
	act( "$T has forsaken you.", ch, NULL, name, TO_CHAR );
	return;
    }
    ...
    if ( victim != NULL && IS_ARENA(ch) )
      act( "$n has escaped!  Hunt $m down!", ch, NULL, victim,
          TO_VICT);
    char_from_room( ch );
    char_to_room( ch, location );
    act( "$n appears in the room.", ch, NULL, NULL, TO_ROOM );
...
void do_enter ( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !( obj = get_obj_list( ch, arg1, ch->in_room->contents ) ) )
      {
       act( "You cannot enter that", ch, NULL, NULL, TO_CHAR );
       return;
      }
    if ( obj->item_type != ITEM_PORTAL )
      {
       send_to_char( "There is nothing to enter here.\n\r", ch );
       return;
      }
    in_room = ch->in_room;
    ...
    {
	act( "You try to enter $p but can't.", ch, obj, NULL, TO_CHAR );
	return;
    }
    act(  "You step into the $p.",ch,obj,NULL, TO_CHAR );
    act( "$n steps into the $p and is gone.", ch, obj, NULL, TO_ROOM );
    char_from_room( ch );
    char_to_room( ch, location );
    send_to_char( "You feel dizzy, and when finally you emerge you are
elsewhere.\n\r", ch );
    act( "$n steps out of the $p before you.", ch, obj, NULL, TO_ROOM);
    do_look( ch, "auto" );
    for ( fch = in_room->people; fch; fch = fch_next )
    {
        fch_next = fch->next_in_room;
	...
	if ( fch->master == ch && fch->position == POS_STANDING )
	{
	    act( "You follow $N through $p.", fch, obj, ch, TO_CHAR );
	    do_enter( fch, arg1 );
	}
    }
    return;
...
void do_bash ( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch))
    {
    	send_to_char("Mobs cannot use bash.\n\r", ch);
    	return;
    }
    one_argument( argument, arg );
...
void do_push(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg1[0] == '\0')
  {
    send_to_char( "Push who what where?", ch);
    return;
  }
...
/*  if(abs(ch->level - victim->level) > 5 && !IS_IMMORTAL(ch))
  {
    send_to_char( "Keep it in level please.\n\r", ch);
    return;
  }
  ...
  if(IS_SET(victim->in_room->room_flags, ROOM_SAFE)
    && ch->clan == 0)
  {
    send_to_char( "Join a clan before messing with clanned people.\n\r", ch);
    return;
  }
...
/*  if ( is_safe(ch, victim) )
  {
    send_to_char( "This room is safe from the likes of you.\n\r", ch);
    return;
  }*/
  ...
  if(ch == victim)
  {
    send_to_char( "You attempt to push yourself, oook.\n\r", ch);
    return;
  }

  if(victim->position != POS_STANDING)
  {
    send_to_char( "Can't push someone who is not standing.\n\r", ch);
    return;
  }
  pexit = ch->in_room->exit[door];
  if(pexit == NULL || IS_SET(pexit->exit_info, EX_CLOSED))
  {
    act( "There is no exit, but you push $M around anyways.", ch, NULL, victim, TO_CHAR);
    act( "$n pushes $N against a wall.", ch, NULL, victim, TO_NOTVICT);
    act( "$n pushes you against a wall, ouch.", ch, NULL, victim, TO_VICT);
    return;
  }
  if ( pexit->to_room->vnum == ROOM_VNUM_SMITHY )
	{
	act( "You slam into $N, but a force field prevents $S entry.", ch, NULL, victim, TO_CHAR );
	act( "$n pushes $N, but a force field makes $M bounce back.", ch, NULL, victim, TO_NOTVICT );
	act( "$n slams into you, but you bounce off a force field.", ch, NULL, victim, TO_VICT );
	return;
	}
...
void do_drag(CHAR_DATA *ch, char *argument)
{
  ...
  if(arg1[0] == '\0')
  {
    send_to_char( "Drag who what where?", ch);
    return;
  }
...
/*  if ( is_safe( ch, victim ) )
  ...
  if(ch == victim)
  {
    send_to_char( "You attempt to drag yourself, oook.\n\r", ch);
    return;
  }

  if(victim->position == POS_STANDING)
  {
    send_to_char( "Can't drag someone who is standing.\n\r", ch);
    return;
  }
  pexit = ch->in_room->exit[door];
  if(pexit == NULL || IS_SET(pexit->exit_info, EX_CLOSED))
  {
    act( "There is no exit, but you drag $M around anyways.", ch, NULL, victim, TO_CHAR);
  ...
  if ( pexit->to_room->vnum == ROOM_VNUM_SMITHY )
    {
    act( "You grap onto $N, but a force field prevents you entry into the smithy.", ch, NULL, victim, TO_CHAR );
    act( "$n attempts to drag $N into the smithy, but a force field stops $m.", ch, NULL, victim, TO_NOTVICT );
    act( "$n attempts to drag you into the smithy, but a force field stops $m.", ch, NULL, victim, TO_VICT );
    return;
    }
  ...
  char_from_room(victim);
  char_to_room(victim, pexit->to_room);
  act( "$N arrives, dragging $n with $M.", victim, NULL, ch, TO_ROOM);
  char_from_room(ch);
...
/*      && ( !IS_AFFECTED( ch, AFF_FLYING ) ) )  */
  {
    act(  "You fall through where the floor should have been!", ch,
        NULL, NULL, TO_CHAR );
    act(  "$n falls down to the room below.", ch, NULL, NULL,
        TO_ROOM );
    damage( ch, ch, 5, TYPE_UNDEFINED );
...
void do_shadow_walk( CHAR_DATA *ch, char *argument )
{
   ...
   one_argument( argument, arg );
    ...
    {
      send_to_char(  "You cannot shadow walk in your condition!\n\r", ch );
      return;
    }
...
/*
    ...
    {
	send_to_char( "The shadows offer no path to that one.\n\r", ch );
	return;
    }
*/
    ...
    if ( !IS_AFFECTED(ch,AFF_HIDE) )
      act( "$n steps into the shadows and is gone.", ch, NULL, NULL, TO_ROOM );
    if ( ch != victim )
    {
     char_from_room( ch );
     char_to_room( ch, victim->in_room );
...
void do_scent( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !can_use_skpell( ch, gsn_scent ) )
	{
	send_to_char(  "Your sense of smell is not keen enough.\n\r", 
			ch );
	return;
	}
  if ( ch->pcdata->learned[gsn_scent] < number_percent( ) )
	{
	send_to_char(  "You sniff around and start to sneeze!.\n\r",
			ch );
	return;
	}
  in_room = ch->in_room;
  for ( dir = 0; dir <= 5; dir++ )
	...
	for ( sch = next_room->people; sch; sch = sch->next_in_room )
	    ...
	    if ( !found )
		{
		send_to_char(  "You pick up the scent of:\n\r", ch );
		found = TRUE;
		}
  ...
  if ( !found )
     send_to_char(  "You did not pick up anything's scent.\n\r", ch );
...
void do_retreat( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !ch->fighting )
	{
	send_to_char(  "Retreat from what? You're not even fighting!\n\r", ch );
	return;
	}
  if ( argument[0] == '\0' )
	{
	send_to_char( "You're retreating, not fleeing!  Provide a direction!\n\r", ch );
	return;
	}
  ...
  if ( !smokebomb )
   {
   send_to_char(  "You need a smoke bomb to retreat!\n\r", ch );
   return;
   }
  if ( IS_AFFECTED( ch, AFF_ANTI_FLEE ) )
   {
   if ( is_affected( ch, gsn_howlfear ) )
     send_to_char(  "You are too frightened to plan an escape.\n\r", ch );
   else
     send_to_char(  "You can't move, how do you expect to retreat?\n\r", ch );
   return;
   }
  send_to_char( "You throw your smoke bomb to the ground and...\n\r", ch );
  act(  "$n throws $s smokebomb to the ground and...", ch, NULL,
	NULL, TO_ROOM );
  if ( number_percent( ) < ch->pcdata->learned[gsn_retreat] )
  {
   ...
   || !pexit->to_room )
    {
    send_to_char(  "Wham!  Ouch! Retreated straight into a wall!\n\r", ch );   
   ...
   send_to_char(  "The bomb failed to explode!", ch );
   act(  "$n's smokebomb duds.", ch, NULL, NULL, TO_ROOM );
