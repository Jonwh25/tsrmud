...
/*
   ...
   {
   send_to_char( "The harsh rays of the sun prevent you from healing.\n\r", ch );
   return 0;
   }
   }
 */
...
/* Ward of Healing */
	...
	send_to_char("The wards of healing soothe your wounds.\n\r", ch);
	gain += 100;
    }
    /* Is the char in a Rented Room? */

    if (IS_SET(ch->in_room->room2_flags, ROOM_RENTED) && ch->money.silver > 0)
	gain *= 2;
    if (IS_SET(ch->act2, PLR2_TENNANT) && ch->money.silver > 0)
	ch->money.silver -= 10;
...
void weather_update(void)
{
	...
	strcat(buf, "The witching hour has passed.\n\r");
	break;

    case 5:
	weather_info.sunlight = SUN_DAWN;
	strcat(buf, "The darkness begins to lift.\n\r");
	break;

    case 6:
	weather_info.sunlight = SUN_MORNING;
	strcat(buf, "The sun rises triumphantly in the east.\n\r");
	break;

    case 11:
	weather_info.sunlight = SUN_NOON;
	strcat(buf, "The sun sits high in its heavenly cradle.\n\r");
	break;

    case 13:
	weather_info.sunlight = SUN_AFTERNOON;
	strcat(buf, "The afternoon sun bakes the land.\n\r");
	break;

    case 18:
	weather_info.sunlight = SUN_DUSK;
	strcat(buf, "The sun begins its slow descent in the west.\n\r");
	break;

    case 19:
	weather_info.sunlight = SUN_DARK;
	strcat(buf, "Darkness spreads its embrace about the land.\n\r");
	break;

    case 24:
	weather_info.sunlight = SUN_MIDNIGHT;
	strcat(buf, "The witching hour has come.\n\r");
	time_info.hour = 0;
	time_info.day++;
...
void char_update(void)
{
    ...
    if (cocount > 0) 
    {
        sprintf(buf,"Copyover in %d ticks !", cocount);
        do_info(coowner, buf);
	...
	if (IS_AFFECTED2(ch, AFF_BERSERK)
		...
		send_to_char("You feel less frenzied.\n\r", ch);
	    else
		send_to_char("The rage leaves you.\n\r", ch);
	...
	for (paf = ch->affected; paf; paf = paf->next) {
		...
		if (!paf->next
			...
			if (skill_table[paf->type].room_msg_off) {
			    act(skill_table[paf->type].room_msg_off,
				ch, NULL, NULL, TO_ROOM);
			}
		    }
		}
...
/*              if ( paf->bitvector == AFF_FLYING ) 
   check_nofloor( ch ); --Angi */
	...
	if (ch->gspell && --ch->gspell->timer <= 0) {
	    send_to_char("You slowly lose your concentration.\n\r", ch);
	    end_gspell(ch);
	}
	if (ch->ctimer > 0)
	    ch->ctimer--;
	...
	if (is_affected(ch, gsn_drowfire)) {
	    send_to_char("The drow fire burns your skin.\n\r", ch);
	    damage(ch, ch, 25, gsn_drowfire);
	}
	if (IS_AFFECTED(ch, AFF_FLAMING)) {
	    send_to_char("Your skin blisters and burns.\n\r", ch);
	    act("$n's body blisters and burns as it is licked in flames.", ch, NULL, NULL, TO_ROOM);
...
/* XOR */
	...
	if (IS_NPC(ch) && ch->summon_timer == 0) {
	    act("$n is consumed by a swirling vortex.",
		ch, NULL, NULL, TO_ROOM);
...
void arena_update()
{
	...
	if (arena.och) {
	    ...
	    sprintf(buf, "%s offering %s a challenge for %d coins.",
		    arena.cch->name, arena.och->name, arena.award);
	    challenge(buf, 0, 0);
	} else
	    challenge("%s offering challenge for %d coins.",
		      (int) (arena.cch->name), arena.award);
	return;
    }
    if (arena.och)
	challenge("%s wimps out and refuses %s's challenge.",
		  (int) (arena.och->name), (int) (arena.cch->name));
    else
	challenge("%s's challenge not accepted.  Opening arena for new challenger.",
		  (int) (arena.cch->name), 0);
    send_to_char("Your challenge was not accepted.  Refunding "
		 "award money.\n\r", arena.cch);
...
void auc_update()
{
	...
	act("$p appears in the hands of $n.", auc_bid, auc_obj, NULL,
	    TO_ROOM);
    } else if (auc_bid) {
#ifdef NEW_MONEY
...
void rdam_update()
{
	...
	if (ch->level < L_APP)
		...
		{
		    send_to_char("Pain shoots through your body.\n\r", ch);
		}
...
/*
   ...
   act(  "Smoke rises from $n under the glarying light of the sun.", ch,
   NULL, NULL, TO_ROOM );
