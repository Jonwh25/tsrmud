...
void do_headbutt(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Wham!  Ouch!  You sure are no minotaur\n\r", ch);
	return;
    }
    if (ch->race_wait > 0)
	return;
    ...
    ch->race_wait = timer;
    act("You slam your head into $N's!", ch, NULL, victim, TO_CHAR);
    act("$n slams his head into yours!", ch, NULL, victim, TO_VICT);
    act("$n slams his head into $N's!", ch, NULL, victim, TO_NOTVICT);
	...
	act("You real from the blow and feel disoriented.",
...
void do_drowfire(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("Drowfire by a non-drow?\n\r", ch);
	return;
    }
    if (argument[0] != '\0') {
    ...
    send_to_char("You are surrounded by a purple outline.\n\r", victim);
    act("$n is surrounded by a purple outline.", victim, NULL, NULL, TO_ROOM);
    if (!victim->fighting)
...
void do_globedarkness(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax: globe <create|dissipate|locate>\n\r", ch);
	return;
    }
    if (!str_cmp(argument, "locate")) {
	for (pd = ch->powered; pd; pd = pd->next) {
	    if (pd->type == gsn_globedark) {
		sprintf(buf, "Globe of Darkness, %s; Cost: %d.\n\r",
	...
	if (!found)
	    send_to_char("You are not sustaining any Globes.\n\r", ch);
	return;
    }
    if (!str_cmp(argument, "dissipate")) {
	if (!is_raffected(ch->in_room, gsn_globedark)) {
	    send_to_char("There is no Globe in this room.\n\r", ch);
	    return;
	}
	for (pd = ch->powered; pd; pd = pd->next) {
	    if (!pd)
		...
		if (pd->room == ch->in_room) {
		    send_to_char("You wave your hand and the globe dissipates.\n\r", ch);
		    act("The globe of darkness dissipates.",
	...
	send_to_char("You are not powering the Globe in this room.\n\r", ch);
	return;
    }
    if (!str_cmp(argument, "create")) {
	if (is_raffected(ch->in_room, gsn_globedark)) {
	    send_to_char("A globe of darkness already sits in this room.\n\r", ch);
	    return;
	}
	...
	globe->short_descr = str_dup("a Globe of Darkness");
	globe->description = str_dup("A Globe of Darkness sits upon the room.");
	...
	send_to_char("You raise your hand and place of globe of darkness upon the area!\n\r", ch);
	act("$n raises his hand and everything goes pitch black!",
	    ch, NULL, NULL, TO_ROOM);
	return;
    }
    do_globedarkness(ch, "");
...
void do_forge(CHAR_DATA * ch, char *argument)
{
	...
	send_to_char("You aren't a dwarf!\n\r", ch);
	return;
    }
    if (argument[0] == '\0') {
	send_to_char("Syntax: Forge <obj> <race> <lvl>\n\r", ch);
	send_to_char("  obj = ring necklace armor helm\n\r", ch);
	send_to_char("        mask leggings boots gauntlets\n\r", ch);
	send_to_char("        gauntlets armplates shield\n\r", ch);
	send_to_char("        belt bracer anklet weapon\n\r", ch);
	send_to_char("  race= any valid race. HELP FORGE RACES\n\r", ch);
	send_to_char("        to see race groupings.\n\r", ch);
	sprintf(buf, "  lvl = minimum 30, maximum %d.\n\r", ch->level);
	send_to_char(buf, ch);
	send_to_char("  BASE cost to make item is: 100 gold * lvl\n\r", ch);
	return;
    }
    if (ch->in_room->vnum != ROOM_VNUM_SMITHY) {
	send_to_char("You cannot forge equipment here.\n\r", ch);
	return;
    }
    ...
    if (!hammer) {
	send_to_char("You must hold a smithy hammer to forge something.\n\r", ch);
	return;
    }
    ...
    if (wear && antirace == 0) {
	send_to_char("Illegal race.  Help RACE for valid race list.\n\r", ch);
	return;
    }
    if (wear) {
#ifdef NEW_MONEY
...
#endif
	    send_to_char("You do not have enough money to create the base item of this level.\n\r", ch);
	    return;
...
void do_spit(CHAR_DATA * ch, char *argument)
{
	...
	case 0:
		...
		act("You spit right in $S eyes!", ch, NULL, victim, TO_CHAR);
		act("$n spit into $N's eyes!", ch, NULL, victim, TO_NOTVICT);
		act("$n spit into your eyes!", ch, NULL, victim, TO_VICT);
		if (!IS_AFFECTED(victim, AFF_BLIND)) {
		    send_to_char("You are blinded!", victim);
	    ...
	    break;
	case 1:
	    act("You spit right in $S mouth!", ch, NULL, victim, TO_CHAR);
	    act("$n spit into $N's mouth!  Gross!", ch, NULL, victim, TO_NOTVICT);
	    act("$n spit into your mouth!", ch, NULL, victim, TO_VICT);
	    send_to_char("The acidic spit burns your mouth and throat.\n\r", victim);
