...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
       ...
       && number_percent ( ) < 10 )
    {
      ...
      send_to_char(  "You stand confused.\n\r", ch );
      return;
    }
...
void damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	if (IS_AFFECTED2( ch, AFF_PHASED ) )
	{
	    ...
	    act( "$n returns from an alternate plane.", ch, NULL, NULL, TO_ROOM );
	}
	/*
	 * Damage modifiers.
	 */
...
*/
  	   ...
  	   )
	{
		...
		return;
	    if ( check_phase( ch, victim ) && dam > 0 )
		return;
       }
    if ( !IS_NPC( ch )
	...
	send_to_char(  "You hit a pressure point!\n\r", ch );
	act(  "$n hit one of $N's pressure points!",
		     ch, NULL, victim, TO_NOTVICT );
	act(  "$n hit you with a precise shot.", 
...
/*            REMOVE_BIT(victim->act, PLR_THIEF);*/
	  ...
	  if ( !IS_NPC( victim ) )
	  {
		...
		{
		REMOVE_BIT( victim->act, PLR_THIEF );
		info( "%s the puny thief gets destroyed by the {rMERCENARY{x %s!",
		...
		info( "%s, the sly thief, has killed the {rMERCENARY{x %s.",
	...
	{
	  sprintf(log_buf, "%s {chas defeated %s {cin the arena!{x",
	          ch->name, victim->name);
	  wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0);
	  log_string(log_buf, CHANNEL_LOG, -1);
...
*/
	    ...
	    act( msg, ch, obj_lose, NULL, TO_CHAR );
            return;	
  	}
    return;
}
...
/* SIGH
  ...
  {
    send_to_char( "You cannot pkill unless you are BOTH pkillers!\n\r", ch );
    return TRUE;
  }
*/
...
/* Err.. we might not want pkill guilds attacking unguilded.. */
  ...
  {
    send_to_char( "You must be clanned to murder.\n\r", ch );
    return TRUE;
  }
  ...
  {
    send_to_char( "You can only murder clanned players.\n\r",ch);
    return TRUE;
  }
...
/* can murder self for testing =) */
  ...
  {
    send_to_char( "You cannot murder your own clan member.\n\r",ch);
    return TRUE;
  }

  if ( !IS_SET(pClan->settings, CLAN_PKILL) )
  {
    send_to_char( "Peaceful clan members cannot murder.\n\r",ch);
    return TRUE;
  }
  ...
  if ( !IS_SET(pClan->settings, CLAN_PKILL ))
  {
    send_to_char( "You may not murder peaceful clan members.\n\r",ch);
    return TRUE;
  }
...
void stop_fighting( CHAR_DATA *ch, bool fBoth )
{
	...
	if ( fch == ch || ( fBoth && fch->fighting == ch ) )
	{
	    ...
	    if ( is_affected( fch, gsn_berserk ) )
	    {
	      ...
	      send_to_char( skill_table[gsn_berserk].msg_off,fch);
	      send_to_char( "\n\r",fch);
	      
	      act( skill_table[gsn_berserk].room_msg_off,fch,
		  NULL, NULL, TO_ROOM);
	    }
	    else if ( is_affected( fch, gsn_frenzy ) )
	    {
	      affect_strip( fch, gsn_frenzy );
	      send_to_char( skill_table[gsn_frenzy].msg_off,fch);
	      send_to_char( "\n\r",fch);
	      act(skill_table[gsn_frenzy].room_msg_off,fch,
		  NULL, NULL, TO_ROOM);
	    }
...
/*    ROOM_INDEX_DATA *location;*/
    ...
    if ( IS_ARENA(ch) )
    {
      ...
      sprintf(log_buf, "%s {chas been awarded %d {ccoins for %s victory.",
...
/* Check if ch has any money, doesn't matter about converting */
	...
	obj_from_char( obj );
       ...
       /*
        * For Joe:Lockers OK this determines if obj is 
        * chars's locker obj. If so drop it in room so it 
        * can be placed back on char. - added 3/15/01 Beo
        */
...
void raw_kill( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    /* For Joe:Lockers. ok locker should have been 
     * dropped in room. Recover locker before char 
     * sent to death room. Added 3/15/01 - Beo
     */
     if ( (obj = get_obj_locker_room(victim,3000)) != NULL ) 
    ...
    /* 
     * Joe:Lockers  Now char is in death room 
     * etdc place locker back on him. Added 3/15/01 - Beo
     */

    if (obj == NULL) return;
    if (obj->pIndexData == NULL) return;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
    ...
    else if ( dam <=   4 ) { vs = "{mscratch{x";	vp = "{mscratches{x";	}
    else if ( dam <=   8 ) { vs = "{mgraze{x";	vp = "{mgrazes{x";		}
    else if ( dam <=  12 ) { vs = "{mhit{x";	vp = "{mhits{x";		}
    else if ( dam <=  16 ) { vs = "{minjure{x";	vp = "{minjures{x";		}
    ...
    else if ( dam <=  32 ) { vs = "{mdevastate{x";	vp = "{mdevastates{x";	}
    else if ( dam <=  36 ) { vs = "{mmaim{x";	vp = "{mmaims{x";		}
    else if ( dam <=  40 ) { vs = "{mMUTILATE{x";	vp = "{mMUTILATES{x";	}
    else if ( dam <=  44 ) { vs = "{mDISEMBOWEL{x";	vp = "{mDISEMBOWELS{x";	}
    else if ( dam <=  48 ) { vs = "{mEVISCERATE{x";	vp = "{mEVISCERATES{x";	}
    else if ( dam <=  52 ) { vs = "{mMASSACRE{x";	vp = "{mMASSACRES{x";	}
    else if ( dam <= 100 ) { vs = "{M*** DEMOLISH ***{x";
			     vp = "{M*** DEMOLISHES ***{x";			}
    else if ( dam <= 150 ) { vs = "{y*** DEVASTATE ***{x";
			     vp = "{y*** DEVASTATES ***{x";			}
    else if ( dam <= 250 ) { vs = "{Y*** OBLITERATE ***{x";
			     vp = "{Y*** OBLITERATES ***{x";		}
    else if ( dam <= 300 ) { vs = "{m=== OBLITERATE ==={x";
			     vp = "{m=== OBLITERATES ==={x";		}
    else if ( dam <= 500 ) { vs = "{r*** ANNIHILATE ***{x";
			     vp = "{r*** ANNIHILATES ***{x";		}
    else if ( dam <= 750 ) { vs = "{G>>> ANNIHILATE <<<{x";
			     vp = "{G>>> ANNIHILATES <<<{x";		}
    else if ( dam <= 1000) { vs = "{B<<< ERADICATE >>>{x";
			     vp = "{B<<< ERADICATES >>>{x";			}
    else                   { vs = "do {rUNSPEAKABLE things to{x";
			     vp = "does {rUNSPEAKABLE things to{x";
}
	...
	{
    sprintf( buf1, "Your {gpoisoned %s {r%s $N%c",  attack, vp, punct );
    sprintf( buf2, "$n's {gpoisoned %s %s you%c", attack, vp, punct );
    sprintf( buf3, "$n's {gpoisoned %s %s $N%c",  attack, vp, punct );
    sprintf( buf4, "Your {gpoisoned %s %s you%c", attack, vp, punct );
    sprintf( buf5, "$n's {gpoisoned %s %s $n%c",  attack, vp, punct );
	}
	else
	{
	    sprintf( buf1, "Your %s {r%s $N%c",  attack, vp, punct );
...
void do_kill( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim == ch )
    {
	send_to_char( "You hit yourself.  Stupid!\n\r", ch );
...
void do_murder( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET( victim->act, PLR_PKILLER ) && !IS_ARENA( victim ) 
	 && !IS_NPC( victim ) ) /* chars can murder mobs */
    {
	send_to_char( "You can only pkill other Pkillers.\n\r", ch );
	return;
    }
...
void do_assasinate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim->fighting )
    {
	send_to_char( "You can't assasinate a fighting person.\n\r", ch );
	return;
    }
...
void do_flee( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_ARENA(ch) )
	  act( "$n has escaped!  After $m!", ch, NULL, victim,
	      TO_VICT);
	ch->in_room = now_in;
...
void do_gouge( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "You'd better leave the martial arts to thieves.\n\r", ch );
	return;
    }
...
void do_high_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's high kick connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's high kick hit $N's head!", ch, NULL, victim, TO_ROOM );
...
void do_jump_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's jump kick connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's jump kick hit $N's head!", ch, NULL, victim, TO_ROOM );
...
void do_spin_kick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's spin kick connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's spin kick hit $N's head!", ch, NULL, victim, TO_ROOM );
...
void do_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's punch connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's punch hit's home!", ch, NULL, victim, TO_ROOM );
...
void do_jab_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's jab connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's jab hit $N's head!", ch, NULL, victim, TO_ROOM );
...
void do_cross_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's cross connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's cross hit $N's head!", ch, NULL, victim, TO_ROOM );
...
void do_roundhouse_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's roundhouse connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's roundhouse hit $N's head!", ch, NULL, victim, TO_ROOM );
...
void do_uppercut_punch( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( number_percent( ) < chance ) || IS_NPC( ch ) )
       {
	 act(  "You hear a crunch as you connect with $N's head.", ch, NULL, victim, TO_CHAR );
	 act(  "$n's uppercut connects firmly with your head!", ch, NULL, victim, TO_VICT );
	 act(  "$n's uppercut hit $N's head!", ch, NULL, victim, TO_ROOM );
...
void do_flury( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( get_eq_char( ch, WEAR_WIELD ) ) || ( get_eq_char( ch, WEAR_WIELD_2 ) ) ||
	 ( get_eq_char( ch, WEAR_HOLD ) ) )
    {
	send_to_char( "You cannot begin such a flury while your hands are full!\n\r",  ch);
	return;
    }
    ...
    act(  "You throw a flury of punches and kicks at $N!", ch, NULL, victim, TO_CHAR );
    act(  "$n throws a flury of punches and kicks at you!", ch, NULL, victim, TO_VICT );
    act(  "$n throws a flury of punches and kicks at $N!", ch, NULL, victim, TO_ROOM );
    ...
    if ( count == 0 )
    {
    act(  "But none of your attacks hit $N!", ch, NULL, victim, TO_CHAR );
    act(  "But none of $n attacks hit you!", ch, NULL, victim, TO_VICT );
    act(  "But none of $n attacks hit $N!", ch, NULL, victim, TO_ROOM );
    }
    return;
}
...
void do_feed( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !is_class( ch, CLASS_VAMPIRE ) )
       {
	 send_to_char(  "You may not feed on the living.\n\r", ch );
	 return;
       }
...
/*
    if ( ch->level < L_APP && ch->class == CLASS_VAMPIRE )
     if ( !IS_SET( ch->in_room->room_flags, ROOM_INDOORS ) )
     {
      if ( time_info.hour > 6 && time_info.hour < 18 )
      {
	send_to_char( "You may not feed during the cursed day.\n\r", ch );
	return;
      }
    }
*/
...
void do_throw( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ( victim = ch->fighting ) == NULL )
  {
	...
	return;
      ...
      {
	send_to_char(  "You cannot throw in that direction.\n\r",
		     ch );
	return;
      }
      ...
      {
	send_to_char(  "You cannot throw through a door.\n\r", ch );
	return;
      }
	       ...
	       IS_SET( pexit->exit_info, EX_CLOSED ) )
	{
	  sprintf( buf, "A $p flys in from $T and hits the %s wall.", 
		   dir_name[dir] );
	  act(  buf, ch, Obj, dir_noun[rev_dir[dir]], TO_ROOM );
	  sprintf( buf, "You throw your $p %d room%s $T, where it hits a wall.",
		   dist, dist > 1 ? "s" : "" );
	  act(  buf, ch, Obj, dir_name[dir], TO_CHAR );
	  char_from_room( ch );
    ...
    if ( dist > 0 )
    {
      char_from_room( ch );
      char_to_room( ch, in_room );
      act(  "A $p flys in from $T and hits $n!", victim, Obj,
	  dir_noun[rev_dir[dir]], TO_NOTVICT );
      act(  "A $p flys in from $T and hits you!", victim, Obj,
  ...
  obj_from_char( Obj );
  obj_to_room( Obj, to_room );
  act(  "$n threw a $p at $N!", ch, Obj, victim, TO_ROOM );
  act(  "You throw your $p at $N.", ch, Obj, victim, TO_CHAR );
  damage( ch, victim, per_type( ch, Obj ), gsn_throw );
...
void do_stun( CHAR_DATA *ch, char *argument )
{
      ...
      number_percent() < (ch->level * 75) / victim->level )
  {
    ...
    act(  "$n stuns $N!", ch, NULL, victim, TO_NOTVICT );
    act(  "$n stuns you!", ch, NULL, victim, TO_VICT );
...
void do_berserk( CHAR_DATA *ch, char *argument )
{
  ...
  send_to_char( "You suddenly go berserk.\n\r",ch);
  act( "$n suddenly goes berserk!", ch, NULL, NULL, TO_ROOM );
...
void do_soulstrike( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ch->hit < dam * 2 )
  {
    send_to_char( "You do not have the strength.\n\r", ch );
    return;
  }
...
/* Don't need a check for update_pos, because of the previous check. */
  ...
  if ( IS_EVIL(ch) )
  {
    send_to_char( "Your soul recoils!\n\r", ch);
    damage(ch, ch, dam, gsn_soulstrike);
    return;
  }

  act(  "Your soul strikes deep into $N.", ch, NULL, victim, TO_CHAR );
  act(  "$n's soul strikes deep into you.", ch, NULL, victim, TO_VICT );
  act(  "$n's soul strikes deep into $N.", ch, NULL, victim, TO_NOTVICT );
...
void do_multiburst( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !can_use_skpell( ch, gsn_multiburst ) )
	{
	send_to_char(  "You're not enough of a mage to do multibursts.\n\r" , ch );
	return;
	}
  if ( IS_STUNNED( ch, STUN_MAGIC ) )
	{
	send_to_char(  "You are too stunned to multiburst.\n\r", ch );
	return;
	}

  if ( ch->pcdata->learned[gsn_multiburst] < number_percent( ) )
	{
	send_to_char(  "You fail your multiburst.\n\r", ch );
	return;
	}
  if ( !ch->fighting )
	{
...
void do_rage( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !can_use_skpell( ch, gsn_rage ) )
	{
	send_to_char(  "You are not enough of a beast.\n\r", ch );
	return;
	}
  if ( !IS_NPC( ch ) && ch->pcdata->learned[gsn_rage] < number_percent( ) )
	{
	send_to_char(  "You cannot summon enough anger.\n\r", ch );
	return;
	}
...
void do_challenge(CHAR_DATA *ch, char *argument)
{
  ...
  if ( IS_NPC(ch) )
  {
    send_to_char(  "NPC's can't fight in the arena.\n\r", ch );
    return;
  }
  if ( rvnum == ROOM_VNUM_HELL
  || rvnum == ROOM_VNUM_RJAIL )
  {
    send_to_char(  "Nice try, but get out the real way.\n\r", ch );
    return;
  }
  if ( arena.fch && arena.sch )
  {
    send_to_char(  "There are already two people fighting in the"
                  " arena.\n\r", ch );
    return;
  }
  if ( arena.cch )
  ...
  {
    send_to_char(  "Syntax: challenge [player] <award>\n\r", ch );
    send_to_char(  " Player is the optional name of a specific person to challenge.\n\r", ch);
    send_to_char(  " Award is at least 1000000 gold coins.\n\r", ch );
    return;
  }
...
/*  if ( award > ( ch->money.gold + (ch->money.silver/SILVER_PER_GOLD) +
  ...
  {
    send_to_char(  "An error has occured.  Please inform an Immortal.\n\r", ch );
    return;
  }
  arena.cch = ch;
  arena.count = 0;
...
#endif
  if ( arena.och )
  sprintf(log_buf, "%s {cchallenges %s {cto a fight in the arena for %d {cgold coins.", 
	  ch->name, arena.och->name, award );
  else
  sprintf(log_buf, "%s {coffers a challenge in the arena for %d {cgold coins.",
          ch->name, award);
/*  wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0); */
  log_string(log_buf, CHANNEL_LOG, -1);
  challenge(log_buf, 0, 0);
  send_to_char(  "Your challenge has been offered.\n\r", ch );
  return;
}
...
void do_accept(CHAR_DATA *ch, char *argument)
{
  ...
  if ( IS_NPC(ch) )
  {
    send_to_char(  "NPC's may not fight in the arena.\n\r", ch );
    return;
  }
  if ( !(cch = arena.cch) )
  {
  ...
  if ( ch == cch )
  {
    send_to_char(  "You can't accept your own challenge!\n\r", ch );
    return;
  }
  if ( ( och = arena.och ) && och != ch )
  {
    send_to_char(  "You are not the one being challenged.\n\r", ch );
    return;
  }
#ifdef NEW_MONEY
...
/*  if ( (ch->money.gold + (ch->money.silver/SILVER_PER_GOLD) +
  ...
  arena.cch = NULL;
  arena.och = NULL;
  send_to_char(  "Your challenge has been accepted.\n\r", cch );
  stop_fighting(cch, FALSE);
  act(  "A pentagram forms around $n and he slowly dissipates.",
  ...
  do_look(ch, "auto");
  sprintf(log_buf, "%s {chas accepted %s{c's challenge.",
          ch->name, cch->name);
/*  wiznet(log_buf, NULL, NULL, WIZ_DEATHS, 0, 0); */
  log_string(log_buf, CHANNEL_LOG, -1);
  challenge(log_buf, 0, 0);
  send_to_char( "Be prepared.\n\r", ch);
  send_to_char( "Be prepared.\n\r", cch);
  return;
}
void do_bite(CHAR_DATA *ch, char *argument)
{
  ...
  {
    send_to_char( "You gnash your teeth wildly.\n\r", ch);
    return;
  }
...
void do_rush( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !can_use_skpell( ch, gsn_rush ) )
	{
	send_to_char(  "You can't pump yourself up enough.\n\r", ch );
	return;
	}
  ...
  send_to_char(  "You pump yourself up and make the adrenaline flow.\n\r", ch );
  act(  "$n seems to be pumped up.", ch, NULL, NULL, TO_ROOM );
  af.type	 = gsn_rush;
  af.level	 = ch->level;
...
void do_howl_of_fear(CHAR_DATA *ch, char *argument)
{
  ...
  if (is_affected(victim, gsn_howlfear) )
    {
    act(  "$N is already scared witless.", ch, NULL, victim, TO_CHAR );
    return;
    }
  ...
  if(number_percent() < ch->pcdata->learned[gsn_howlfear])
  {
    ...
    act(  "Your chilling howl instills fear in $N.", 
         ch, NULL, victim, TO_CHAR );
    act(  "You cringe in terror at the sound of $n's howl.",
	 ch, NULL, victim, TO_VICT );
    act(  "$n howls and $N cringes back in terror.",
	 ch, NULL, victim, TO_NOTVICT );
    if ( !victim->fighting )
	set_fighting( victim, ch );
  }
...
void do_frenzy( CHAR_DATA *ch, char *argument )
{
  ...
  send_to_char( "You go into a frenzy.\n\r",ch);
  act( "$n suddenly goes into a frenzy", ch, NULL, NULL, TO_ROOM );
...
void do_reflex( CHAR_DATA *ch, char *argument )
{
  ...
  send_to_char(  "You feel yourself more agile.\n\r", ch );
  act(  "$n is moving more fluidly.", ch, NULL, NULL, TO_ROOM );
  af.type	 = gsn_reflex;
  af.level	 = ch->level;
  af.duration	 = ch->level / 4;
  af.location	 = APPLY_DEX;
...
void do_bladepalm( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !can_use_skpell( ch, gsn_bladepalm ) )
	{
	send_to_char(  "You know nothing of martial arts.\n\r", ch );
	return;
	}
  if ( get_eq_char( ch, WEAR_WIELD ) && get_eq_char( ch, WEAR_WIELD_2 ) )
	{
	send_to_char(  "You need at least one free hand to perform a blade palm.\n\r", ch );
	return;
	}
  if ( argument[0] != '\0' )
	{
...
void do_flyingkick( CHAR_DATA *ch, char *argument )
{
  ...
  && !can_use_skpell( ch, gsn_flykick ) )
    {
    send_to_char(  "You do not hav knowledge of martial arts.\n\r", ch );
    return;
    }
 if ( !ch->fighting )
    {
...
void do_nerve( CHAR_DATA *ch, char *argument )
{
  ...
  {
	AFFECT_DATA af;
	act(  "You pinch $N's arm nerves!", ch, NULL, victim, TO_CHAR );
	act(  "$n pinched your arm nerves.", ch, NULL, victim, TO_VICT );
	act(  "$n pinched $N's arm nerves.", ch, NULL, victim, TO_NOTVICT );
	...
	if ( (hold = get_eq_char( victim, WEAR_HOLD )) )
		...
		if ( !IS_SIMM( victim, IMM_NERVE ) )
		  {
		  act(  "$N drops $S $p.", ch, hold, victim, TO_CHAR );
    		  act(  "$N drops $S $p.", ch, hold, victim, TO_NOTVICT );
		  act(  "You drop your $p.", ch, hold, victim, TO_VICT );
		  obj_from_char( hold );
		  obj_to_room( hold, victim->in_room );
		  }
		else
		  act(  "$N grips $S $p tightly.", ch, hold, victim, TO_NOTVICT );
		}
  ...
  if ( ( obj = victim->carrying ) && !saves_spell( ch->level, victim ) )
	...
	if ( obj->item_type == ITEM_CONTAINER )
	  ...
	  for ( potion = obj->contains; potion; potion = potion_next )
		...
		{
		extract_obj( potion );
		act( "You feel something explode from within $p.",
	...
	if ( number_bits( 2 ) != 0 )
	  {
	  act( "$p vibrates and explodes!", victim, obj, NULL, TO_CHAR );
	  extract_obj( obj );
	  blown++;
	  }
	}
...
/* -------------------------------------------
 * Phase Skill for ROM24, 1.00 - 3/28/98
 * -------------------------------------------
 * The phase skill is a defensive skill similar
 * to dodge that allows the player to shift
 * out of phase to avoid incoming attacks.
 *
 ...
 * [bribe@erols.com]
 *
 * Phase skill added by Beowolf 3/16/01
 * ------------------------------------------- */
