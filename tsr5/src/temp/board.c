...
{ "Announce", 	"Announcements from Immortals",  0,     L_APP, "all", DEF_NORMAL, 60, NULL, FALSE },
{ "Bugs",	  	"Typos, bugs, errors",		     0,     1,     "imm", DEF_NORMAL, 60, NULL, FALSE },
{ "Personal", 	"Personal messages",			 0,     1,     "all", DEF_EXCLUDE,28, NULL, FALSE },
{ "Immortal",   "Immortal board",			 L_APP, L_APP, "imms", DEF_INCLUDE,28,NULL, FALSE }
		...
		case DEF_EXCLUDE: /* forced exclude */
			if (!buf[0])
			{
				send_to_char ("You must specify a recipient.\n\r"
									BOLD YELLOW "To" NO_COLOR ":      ", d->character);
				return;
			}
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!buf[0])		
	{
		...
		send_to_char ( BOLD YELLOW "Subject" NO_COLOR ": ", d->character);
	}
	else  if (strlen(buf)>60)
	{
		send_to_char ( "No, no. This is just the Subject. You're not writing the note yet. Twit.\n\r", d->character);
	}
	else
...
void handle_con_note_expire(DESCRIPTOR_DATA *d, char * argument)
{
	...
	if (!buf[0]) /* assume default expire */
		...
		if (!is_number(buf))
		{
			...
			send_to_char (BOLD YELLOW "Expire" NO_COLOR ":  ",d->character);
			return;
		}
		else
		{
