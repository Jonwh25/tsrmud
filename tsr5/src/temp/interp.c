...
/* XOR */
    ...
    {"beep", do_beep, POS_STANDING, 0, LOG_NORMAL},
    ...
    {"itell", do_itell, POS_RESTING, 0, LOG_NORMAL},
    ...
    /*
       * Scan code added for TSR by Akasha.  03-11-2001
     */
    ...
    {"who", do_huntwho, POS_DEAD, 0, LOG_NORMAL},
//    {"hwho", do_huntwho, POS_DEAD, 0, LOG_NORMAL},
...
/*    { "qjoin",                do_questflag,   POS_DEAD, 0,LOG_NORMAL }, */
    ...
    /*  {"chat", do_chat, POS_SLEEPING, 0, LOG_NORMAL},    Chat channel removed  */
    ...
    {"ireply", do_ireply, POS_SLEEPING, 0, LOG_NORMAL},
    {"say", do_say, POS_RESTING, 0, LOG_NORMAL},
    {"osay", do_osay, POS_RESTING, 0, LOG_NORMAL},
    {"'", do_say, POS_RESTING, 0, LOG_NORMAL},
    {"gossip", do_gossip, POS_RESTING, 3, LOG_NORMAL},
    {"quote", do_quote, POS_RESTING, 3, LOG_NORMAL},
    {"gratz", do_gratz, POS_RESTING, 3, LOG_NORMAL},
...
/*    { "vent",           do_vent,        POS_DEAD,        0,  LOG_NORMAL },
   { "/",              do_vent,        POS_DEAD,        0,  LOG_NORMAL }, */
    ...
    {"chest", do_chest, POS_RESTING, 0, LOG_NORMAL},
  ...
  {"roundhouse punch", do_roundhouse_punch, POS_FIGHTING, 0, LOG_NORMAL},
    ...
    {"coding", do_coding, POS_DEAD, 0, LOG_NORMAL},
    ...
    {"rental", do_rental, POS_DEAD, 0, LOG_NORMAL},
    {"checkout", do_checkout, POS_DEAD, 0, LOG_NORMAL},
...
/*    { "load",           do_load,        POS_DEAD,  L_DIR,  LOG_NORMAL }, */
    ...
    {"recent", do_recent, POS_DEAD, L_DIR, LOG_ALWAYS},
    {"passwd", do_resetpass, POS_DEAD, L_DIR, LOG_ALWAYS},
    {"copyover", do_cocount, POS_DEAD, L_DIR, LOG_ALWAYS},
    ...
    {"poof", do_poof, POS_DEAD, L_APP, LOG_NORMAL},
    ...
    {"countcmds", do_countcmds, POS_DEAD, L_IMP, LOG_NORMAL},
    {"cset", do_cset, POS_DEAD, L_DIR, LOG_BUILD},
    {"deny", do_deny, POS_DEAD, L_GOD, LOG_ALWAYS},
    {"ds", do_ds, POS_DEAD, L_IMP, LOG_NORMAL},
    ...
    {"empower", do_empower, POS_DEAD, L_CON, LOG_ALWAYS},
    {"force", do_force, POS_DEAD, L_DEM, LOG_ALWAYS},
    {"freeze", do_freeze, POS_DEAD, L_GOD, LOG_ALWAYS},
    {"goto", do_goto, POS_DEAD, L_APP, LOG_NORMAL},
    {"exlist", do_exlist, POS_DEAD, L_APP, LOG_NORMAL},
    {"vlist", do_vlist, POS_DEAD, L_APP, LOG_NORMAL},
    {"rename", do_rename, POS_DEAD, L_APP, LOG_NORMAL},
    {"hero", do_hero, POS_DEAD, 100, LOG_NORMAL},
    {"hlist", do_hlist, POS_DEAD, L_APP, LOG_NORMAL},
    {"hnumber", do_hnumber, POS_DEAD, L_APP, LOG_NORMAL},
    ...
    {"aset", do_aset, POS_DEAD, L_APP, LOG_NORMAL},
    {"authorize", do_authorize, POS_DEAD, L_APP, LOG_NORMAL},
    {"avatar", do_avatar, POS_DEAD, L_APP, LOG_NORMAL},
    ...
    {"sockets", do_sockets, POS_DEAD, L_DIR, LOG_NORMAL},
...
/*
   ...
   {   "oral",
   "Answer: ORAL SEX!! And lots of it!",
   "$n yells, 'Answer: ORAL SEX!! And lots of it!'",
   "Answer: ORAL SEX!! And lots of it!", 
   "$n yells, 'Answer: ORAL SEX!! And lots of it!'",  
   "$n yells, 'Answer: ORAL SEX!! And lots of it!'",         
   "You remind yourself of the answer, ORAL SEX!",
   "$n yells, 'Answer: ORAL SEX!! And lots of it!'"
   },
 ...
 */
  ...
  /*  {
     ...
     {
     "pray",
     "You feel righteous, and maybe a little barlish.",
     ...
     {
     "tap",
     "You tap your bart impatiently.",
     "$n taps $s bart impatiently.",
     "You tap your bart impatiently.  Will $E ever be ready?",
     "$n taps $s bart impatiently as $e waits for $N.",
     "$n taps $s bart impatiently as $e waits for you.",
...
void interpret(CHAR_DATA * ch, char *argument)
{
    ...
    /* MemoryChecks by Garry Turkington <gary@reddwarf.qub.ac.uk> */
	...
	send_to_char("You're too stunned to do anything!\n\r", ch);
	return;
    }
    /* Check memory */
     ...
     */
    if (IS_AFFECTED(ch, AFF_HIDE)) {	/*Things you can do while hiding */
	if (str_cmp(cmd_table[cmd].name, "pick") &&
	    ...
	    str_cmp(cmd_table[cmd].name, "scan")) {	/* Added for scan */
	...
	if (!found || (str_cmp(cmd_table[cmd].name, "flee") &&
		       ...
		       str_cmp(cmd_table[cmd].name, "score") &&
		       str_cmp(cmd_table[cmd].name, "scan"))) {		/* added for scan */
	    send_to_char("You cannot do that in such a rage!\n\r", ch);
	    return;
	}
    }
    if (!found) {
 ...
 */
	...
	send_to_char("What do you think you are, a BARD???\n\r", ch);
	return;
    } else if (!strcmp(cmd_table[cmd].name, "cast")
	       && prime_class(ch) == CLASS_BARD) {
	send_to_char("You're a bard, SING!\n\r", ch);
	return;
    }
...
/** Do arena_master so we can look at arg and send info over channel then we come
    ...
    /* 
     * Flag the WIZ_MEMCHECK channel if bad memory 
     * MemoryChecks by Garry Turkington <gary@reddwarf.qub.ac.uk> 
     */
    if (string_count < nAllocString) {
	sprintf(buf, "Memcheck : Increase in strings :: %s : %s", ch->name, cmd_copy);
	wiznet(buf, NULL, NULL, WIZ_MEMCHECK, 0, get_trust(ch));
    }
    if (perm_count < nAllocPerm) {
	sprintf(buf, "Increase in perms :: %s : %s", ch->name, cmd_copy);
	wiznet(buf, NULL, NULL, WIZ_MEMCHECK, 0, get_trust(ch));
    }
    return;
}
...
/*
   ...
   break;
	...
	if (!(victim = get_char_world(ch, arg)))
		...
		send_to_char("What do you think this is, a vacation?\n\r", ch);
	    } else {
	...
	if (!IS_NPC(ch)
	    ...
	    case 7:
	    case 8:
		act(pSocial->char_found,
		    victim, NULL, ch, TO_CHAR);
		act(pSocial->vict_found,
		    victim, NULL, ch, TO_VICT);
		act(pSocial->others_found,
...
/*
 * Given a string like 14.bar, return 14 and 'bar'
 */
...
void do_clear(CHAR_DATA * ch, char *argument)
{
    ...
    if (!ch->pcdata->alias_list) {
	send_to_char("You have no alias' to clear.\n\r", ch);
	return;
    }
    ...
    send_to_char("All aliases have been erased.\n\r", ch);
...
void do_alias(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0') {
	if (!ch->pcdata->alias_list) {
	    send_to_char("You have no alias' set.\n\r", ch);
	    return;
	}
	send_to_char("Your currently set alias' are:\n\r", ch);

	for (al = ch->pcdata->alias_list; al; al = al->next) {
	    sprintf(log_buf, "  %-10s   %s\n\r", al->old, al->new);
	...
	send_to_char("You have no command alias' to remove!\n\r", ch);
	return;
    }
    if (arg1[0] != '\0') {
	for (oAl = ch->pcdata->alias_list; oAl; oAl = oAl->next) {
	    if (is_name(NULL, arg1, oAl->old) || !str_prefix(arg, arg1)) {
		send_to_char("You cannot alias other alias' into your [new] string.\n\r", ch);
		return;
	    }
	}
    }
    ...
    if (arg1[0] == '\0') {
	if (!al) {
	    send_to_char("Alias does not exist.\n\r", ch);
	    return;
	}
...
void do_countcmds(CHAR_DATA * ch, char *argument)
{
    char buf[MAX_STRING_LENGTH];
    int scnt;
    int skill[MAX_CLASS];
...
void do_afk(CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char("You announce that you are going AFK.\n\r",
		 ch);
    act("$n announces, 'I am going AFK.'", ch, NULL, NULL,
	TO_ROOM);
...
void do_pkill(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char(
			"Syntax: pkill yes    To choose to become a PKILLER forever!\n\r", ch);
	send_to_char("This is NOT a reversible choice. Type HELP PK.\n\r", ch);
	return;
    }
    if (!str_cmp(argument, "yes")) {
	if (ch->level < 20) {
	    send_to_char("You must be at least level 20 to choose the path of PKILL.\n\r", ch);
	    return;
	}
	if (IS_SET(ch->act, PLR_PKILLER)) {
	    send_to_char("You have already chosen the path of pkilling.\n\r", ch);
	    return;
	}
	send_to_char(
