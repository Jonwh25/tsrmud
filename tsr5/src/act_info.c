...
{
    "{Y<{wused as light{Y>     {x",
    "{Y<{wworn on finger{Y>    {x",
    "{Y<{wworn on finger{Y>    {x",
    "{Y<{wworn around neck{Y>  {x",
    "{Y<{wworn around neck{Y>  {x",
    "{Y<{wworn on body{Y>      {x",
    "{Y<{wworn on head{Y>      {x",
    "{Y<{wworn in eyes{Y>      {x",
    ...
    "{Y<{wworn on legs{Y>      {x",
    "{Y<{wworn on feet{Y>      {x",
    "{Y<{wworn on hands{Y>     {x",
    "{Y<{wworn on arms{Y>      {x",
    "{Y<{wworn as shield{Y>    {x",
    "{Y<{wworn about body{Y>   {x",
    "{Y<{wworn about waist{Y>  {x",
    "{Y<{wworn around wrist{Y> {x",
    "{Y<{wworn around wrist{Y> {x",
    "{Y<{wwielded{Y>           {x",
    "{Y<{wdual wielded{Y>      {x",
    "{Y<{wheld{Y>              {x",
...
void show_char_to_char_0(CHAR_DATA * victim, CHAR_DATA * ch)
{
	...
	strcat(buf2, "'s body is veiled in a glowing {Wwhite mist{x.\n\r");
	buf2[4] = UPPER(buf2[4]);
	...
	strcat(buf2, "'s body is enveloped in a {Kblack veil of mist{x.\n\r");
	buf2[4] = UPPER(buf2[4]);
	send_to_char(buf2, ch);
	buf2[0] = '\0';
    }
	...
	strcat(buf2, "'s body is encased by an unfelt heat.\n\r");
	buf2[4] = UPPER(buf2[4]);
	send_to_char(buf2, ch);
	buf2[0] = '\0';
    }
...
void show_char_to_char_1(CHAR_DATA * victim, CHAR_DATA * ch, char *argument)
{
	...
	act("{w$n looks at $N.{x", ch, NULL, victim, TO_NOTVICT);
    }
/* Put the check here for argument = storage.. display their storage 
   and then return w/o showing char stuff   --Angi */
	...
	if (IS_NPC(victim)) {
	    send_to_char("{wNPC's do not have items in storage.{x\n\r", ch);
	} else {
	    sprintf(buf, "{w%s's storage box contains:{x\n\r", victim->name);
	    send_to_char(buf, ch);
	...
	if (!(obj = get_obj_here(victim, argument))) {
	    sprintf(buf, "{w%s is not carrying that item.{x\n\r", victim->name);
	    send_to_char(buf, ch);
	    return;
	} else {
	    switch (obj->item_type) {
	    default:
		send_to_char("{wThat is not a container.{x\n\r", ch);
		break;

	    case ITEM_DRINK_CON:
		if (obj->value[1] <= 0) {
		    send_to_char("{wIt is empty.{x\n\r", ch);
		    break;
		}
		sprintf(buf, "{w%s %s has is %s full of a %s liquid.{x\n\r",
			capitalize(obj->short_descr), victim->name,
			obj->value[1] < obj->value[0] / 4
			? "{wless than{x" :
			obj->value[1] < 3 * obj->value[0] / 4
			? "{wabout{x" : "{wmore than{x",
...
/*            if ( IS_SET( obj->value[1], CONT_CLOSED ) )   
   {
   send_to_char( "{wIt is closed.{x\n\r", ch );
   break;
   } <so doesn't matter if its closed, look anyways> */

		sprintf(buf, "{w%s %s has contains:{x\n\r",
			capitalize(obj->short_descr), victim->name);
		send_to_char(buf, ch);
/*            act( "{w$p contains:{x", ch, obj, NULL, TO_CHAR ); */
	...
	act("{wYou see nothing special about $M.{x", ch, NULL, victim, TO_CHAR);
    }
	...
	if ((IS_IMMORTAL(victim)) && (!IS_IMMORTAL(ch)))
	    send_to_char("\n\r{wYou cannot peek into an Immortal's inventory.{x\n\r", ch);
	else {
	    send_to_char("\n\r{wYou peek at the inventory:{x\n\r", ch);
...
/*          || ( fGuildRestrict && str_cmp( wch->guild->name,
   guild_table[iGuild].name ) )   */
	 ...
	 * Format it up.
	 */
        /* REMOVED if (wch->level < LEVEL_HERO) {
        ...
        if (wch->level < LEVEL_HERO) {
            ...
            sprintf(buf + strlen(buf), "[%*s%-15s%*s] ",
                    len, "", class, len2, "");
        } else {
...
/*        wch->name, ( *wch->pcdata->lname != 39 ) ? " " : "",  */
	    ...
	    sprintf(buf + strlen(buf), "{C[{R%s{C]{x {r[%s]{x %s%s%s %s%s ",
		    IS_SET(wch->act, PLR_PKILLER) ? "{RK{x" : " ",
	    /*  IS_SET(wch->act, PLR_SOUND) ? "{GS{x" : " ", */
		    lvl, IS_SET(wch->act, PLR_AFK) ? "{m<{RA{WF{BK{m>{x" : "",
		  IS_SET(wch->act, PLR_WIZINVIS) ? "{Y({CWizi{Y){x" : "",
		IS_SET(wch->act, PLR_CLOAKED) ? "{Y({CCloaked{Y){x" : "",
	...
	send_to_char("{wThere is no class/race/clan/person by that name in the game.{x\n\r", ch);
    return;
}
...
/* Outdated by my changes to normal who ( besides, i'm too lazy to type
   ...
   *
   if ( wch->level < LEVEL_HERO )
   sprintf( buf + strlen( buf ), "{R[{Y%2d %s %s{R]{x ",
   wch->level, race, class );
   else
   sprintf( buf + strlen( buf ), "{R[{Y%s{R]{x ", class);
   send_to_char(  buf, ch );
...
void do_where(CHAR_DATA * ch, char *argument)
{
    ...
    if (arg[0] == '\0') {
        sprintf(buf,"You are currently in %29.29s.\n\r", &ch->in_room->area->name[8]);
        send_to_char(buf,ch);
...
void do_finger(CHAR_DATA * ch, char *argument)
{
    ...
    if (IS_IMMORTAL(victim) && !IS_IMMORTAL(ch)) {
        act("I don't think $N would like that!", ch, NULL, victim, TO_CHAR);
        return;  
    }
