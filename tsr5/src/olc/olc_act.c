...
REDIT( redit_show )
{
    ...
    for ( cnt = 0; cnt < MAX_DIR; cnt++ )
    {
		...
		pRoom->exit[cnt]->to_room ? pRoom->exit[cnt]->to_room->vnum : 0,      /* ROM OLC */
		pRoom->exit[cnt]->key );
	    strcat( buf1, buf );
