...
void do_raffect(CHAR_DATA * ch, char *argument)
{
    ...
    if (!ch->powered)
	send_to_char("You are not powering any room affects.\n\r", ch);

    for (pd = ch->powered; pd; pd = pd->next) {
	sprintf(buf, "%s, Mana Cost: %d, Room: %s.\n\r",
...
void loc_off_raf(CHAR_DATA * ch, int gsn_raf, bool rOff)
{
	...
	if (!found) {
	    sprintf(buf, "You are not sustaining any %s.\n\r",
	...
	sprintf(buf, "There is no %s in this room.\n\r",
		skill_table[gsn_raf].name);
	send_to_char(buf, ch);
    ...
    sprintf(buf, "You are not powering the %s in this room.\n\r",
	    skill_table[gsn_raf].name);
    send_to_char(buf, ch);
...
void do_ward(CHAR_DATA * ch, char *argument)
{
    ...
    if (argument[0] == '\0') {
	send_to_char("Syntax: ward <type> <on|off|locate>\n\r", ch);
	return;
    }
    wname = shortd = longd = "";
	...
	send_to_char("This room is already warded to heal.\n\r", ch);
	return;
    }
