...
void do_noteroom( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(arg, "board") || !str_cmp(arg, "to") 
    ...
    {
      send_to_char("You can only use write commands on a board.\n\r", ch);
      return;
    }
    else
    {
...
void do_note( CHAR_DATA *ch, char *arg_passed, bool IS_MAIL )
{
	...
	if ( !can_read( ch, board ) )
	{
	    ...
	    if (first_list < 1)
	    {
		send_to_char( "You can't read a board before 1!\n\r", ch);
		return;
	    }
	}
	    ...
	    act( AT_ACTION, "$n glances over the board.", ch, NULL, NULL, TO_ROOM );
	    return;
	}
	else
	{
	...
	{
	    send_to_char( "write read which number?\n\r", ch );
	    return;
	}
	...
	if (!IS_MAIL)
 	{
		...
		if ( vnum == anum || fAll )
		{
		    ...
		    act( AT_ACTION, "$n reads the board.", ch, NULL, NULL, TO_ROOM );
		    return;
		}
	    }
	...
	{
            send_to_char( "Write vote which number?\n\r", ch );
            return;
	}
	...
	*/
	...
	if ( !str_cmp( arg_passed, "close" ) )
	{
	    ...
	    act( AT_ACTION, "$n closes voting on a message.", ch, NULL, NULL, TO_ROOM );
	...
	{
	    send_to_char( "Voting is not open on this message.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "You have already voted on this message.\n\r", ch );
	    return;
	}
	if ( !str_cmp( arg_passed, "yes" ) )
	{
	    ...
	    act( AT_ACTION, "$n votes on a message.", ch, NULL, NULL, TO_ROOM );
	    send_to_char( "Ok.\n\r", ch );
	    write_board( board );
	    return;
	}  
	...
	{
	    send_to_char( "You cannot write a message from within another command.\n\r", ch );
	    return;
	}
	if (get_trust (ch) < sysdata.write_mail_free)
	{
	    quill = find_quill( ch );
            if (!quill)
	    {
		send_to_char("You need a quill to write a message.\n\r", ch);
		return;
	    }
	    if ( quill->value[0] < 1 )
	    {
	...
	||     paper->item_type != ITEM_PAPER )
	{
	    if (get_trust(ch) < sysdata.write_mail_free )
	    {
		send_to_char("You need to be holding a fresh piece of parchment to write a message.\n\r", ch);
		return;
	    }
	...
	||     paper->item_type != ITEM_PAPER )
	{
	    send_to_char("You are not holding a message.\n\r", ch);
	    return;
	}
	...
	if ( paper->value[0] == 0 )
	{
	    send_to_char("There is nothing written on this message.\n\r", ch);
	    return;
	}

	if ( paper->value[1] == 0 )
	{
	    send_to_char("This message has no subject.\n\r", ch);
	    return;
	}
	...
	if ( !board )
	{
	    send_to_char( "There is no bulletin board here to post your message on.\n\r", ch );
	    return;
	}
	if ( !can_post( ch, board ) ) 
	{
	    send_to_char( "A magical force prevents you from posting your message here...\n\r", ch );
	    return;
	}

	if ( board->num_posts >= board->max_posts )
	{
	    send_to_char( "There is no room on this board to post your message.\n\r", ch );
	    return;
	}

	act( AT_ACTION, "$n posts a message.", ch, NULL, NULL, TO_ROOM );
	...
	board->num_posts++;
        write_board( board );
	send_to_char( "You post your message on the board.\n\r", ch );
	extract_obj( paper );
	return;
    }
	...
	if ( !board )
	{
	    send_to_char( "There is no board here to take a message from!\n\r", ch );
	    return;
	}
	if ( !str_cmp( arg, "take" ) )
	  take = 1;
	else if ( !str_cmp( arg, "copy" ) )
	{
	  if ( !IS_IMMORTAL(ch) )
	  {
	    send_to_char( "Huh?  Type 'help write' for usage.\n\r", ch );
	    return;
	  }
	  take = 2;
	}
	...
	for ( pnote = board->first_note; pnote; pnote = pnote->next )
	{
 		...
 		if ( take != 0 )
		{
		    ...
		    paper->value[1] = 2;
	 	    paper->value[2] = 2;
		    sprintf(short_desc_buf, "a message from %s to %s",
			pnote->sender, pnote->to_list);
		    STRFREE(paper->short_descr);
		    paper->short_descr = STRALLOC(short_desc_buf);
		    sprintf(long_desc_buf, "A message from %s to %s lies on the ground.",
			pnote->sender, pnote->to_list);
		...
		if ( take == 1 )
		{
		    act( AT_ACTION, "$n takes a message.", ch, NULL, NULL, TO_ROOM );
