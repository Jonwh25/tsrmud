...
"nosummon", "noastral", "teleport", "teleshowdesc", "nofloor", "r1", "r2",
"r3", "r4", "r5", "prototype", "r6"
};
...
"secretive", "polymorphed", "mobinvis", "noassist", "r9", "r10", "r11", "r12",
"r13", "prototype", "r14" };
...
"wield", "hold", "dual_wield", "ears", "eyes", "missile_wield", "belt"
};
...
void do_goto( CHAR_DATA *ch, char *argument )
{
    ...
    /* Fixed poofout by Beowolf */
    if ( !IS_SET(ch->act, PLR_WIZINVIS) )
      if (ch->pcdata && ch->pcdata->bamfout[0] != '\0')
 	act( AT_IMMORT, "$t", ch, ch->pcdata->bamfout, rch, TO_ROOM );
      else
	act( AT_IMMORT,"$n leaves in a swirling mist.",ch, NULL, rch, TO_ROOM );
    ...
    /* Fixed poofin by Beowolf */
    if ( !IS_SET(ch->act, PLR_WIZINVIS) )
      if (ch->pcdata && ch->pcdata->bamfin[0] != '\0')
 	act( AT_IMMORT,"$t", ch, ch->pcdata->bamfin, rch, TO_ROOM );
      else
	act(AT_IMMORT,"$n appears in a swirling mist.",ch, NULL, rch, TO_ROOM );
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	case SUB_MOB_DESC:
	  if ( !ch->dest_buf )
	  {
		send_to_char( "Fatal error: report to Beowolf.\n\r", ch );
    ...
    if ( !victim && get_trust( ch ) < LEVEL_ACOLYTE )
    {
...
void do_oset( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !obj && get_trust(ch) < LEVEL_AVATAR )
    {
...
void edit_buffer( CHAR_DATA *ch, char *argument )
{
   ...
   if ( argument[0] == '/' || argument[0] == '\\' )
   {
	...
	if ( get_trust(ch) > LEVEL_BUILD && !str_cmp( cmd+1, "!" ) )
	{
	    DO_FUN *last_cmd;
...
void assign_area( CHAR_DATA *ch )
{
	...
	&&   ch->pcdata->r_range_hi )
	{
	  ...
	  if ( !tarea )
	  {
	    ...
	    wiznet( buf, ch, NULL, WIZ_BUILD, 0, 0); 
            log_string(buf);
...
void fold_area( AREA_DATA *tarea, char *filename, bool install )
{
    ...
    wiznet( buf, NULL , NULL, WIZ_BUILD, 0, 0); 
    log_string(buf);
...
/* Golias -- hack on rlist to allow 3rd argument for room flag. */
void do_rlist( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(ch) || get_trust( ch ) < LEVEL_NEOPHYTE || !ch->pcdata
    || ( !ch->pcdata->area && get_trust( ch ) < LEVEL_NEOPHYTE ) )
    {
	...
	if (checkroomflag)
	{
	  if(IS_SET( room->room_flags, (1 << rflag ) ) )
        ch_printf( ch, "(%s) %5d) %s\n\r",r_flags[rflag], vnum, room->name );
	}
	else
        ch_printf( ch, "%5d) %s\n\r", vnum, room->name );
...
void do_olist( CHAR_DATA *ch, char *argument )
{
     ...
     */
    if ( IS_NPC(ch) || get_trust( ch ) < LEVEL_AVATAR || !ch->pcdata
    || ( !ch->pcdata->area && get_trust( ch ) < LEVEL_ACOLYTE ) )
    {
	send_to_char( "You don't have an assigned area.\n\r", ch );
	return;
    }
