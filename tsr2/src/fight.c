...
*/
	...
	if ( !npcvict )
	{
	    ...
	    wiznet( log_buf, ch, NULL, WIZ_DEATHS, 0, 0);
            sound_to_all(AT_SAY, "!!SOUND(Tsr/Pdeath V=100)", ECHOTAR_ALL); 
...
/* Take care of polymorphed chars */
 ...
 /* death_cry( victim ); */
    ...
    if (IS_SET(victim->pcdata->flags, PCFLAG_SOUND))
      send_to_char("!!SOUND(Tsr/arggg V=100)\n\r", victim);
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( gch->level - lch->level >  8 )
	{
        ...
        if (IS_SET(ch->pcdata->flags, PCFLAG_QUEST)&&IS_NPC(victim))
        {
...
void do_kill( CHAR_DATA *ch, char *argument )
{
    ...
    */
    ...
    {
       send_to_char( "The Gods forbid you from killing this creature.\n\r", ch );
       return;
    }
...
void do_flee( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC(ch) )
	{
	    ...
	    int level_ratio;	    
	    los = (exp_level(ch, ch->level+1) - exp_level(ch, ch->level)) * 0.03;
	    ...
	    act( AT_FLEE, buf, ch, NULL, NULL, TO_CHAR );
            if (IS_SET(ch->pcdata->flags, PCFLAG_SOUND))
              send_to_char("!!SOUND(Tsr/runaway V=100)\n\r", ch);
	    gain_exp( ch, 0 - los );
