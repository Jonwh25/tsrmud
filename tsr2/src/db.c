...
void make_wizlist( )
{
  ...
  buf[0] = '\0';
  unlink( WIZLIST_FILE );
  towizfile( "=-=-=-=-=-=-=-=-=-=-= GODS OF THE SHADOW REALMS =-=-=-=-=-=-=-=-=" );
  ilevel = 65535;
  for ( wiz = first_wiz; wiz; wiz = wiz->next )
  {
	...
	case MAX_LEVEL - 12: towizfile( " Master Builders" );	break;
	case MAX_LEVEL - 13: towizfile( " Builders" );		break;
	case MAX_LEVEL - 14: towizfile( " Novice Builders" );	break;
