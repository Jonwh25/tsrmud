...
/* Orginal Code By Altrag - Added in By Beowolf on 8-9-97 */
void do_donate(CHAR_DATA *ch, char *argument)
{
  ...
  if (!(droom = get_room_index(ROOM_VNUM_DONATE)))
  {
    send_to_char("Theres nowhere to donate to.\n\r", ch );
    return;
  }
  obj_from_char(obj);
  obj_to_room(obj, droom);
  send_to_char("Item Sent!\n\r", ch);
  act(AT_RED, "$n donates $p.", ch, obj, NULL, TO_ROOM);
  return;
}
