...
void violence_update( void )
{
	...
	if ( IS_FLAG2 (ch, AFF_FEAR) )
	{
	    switch(ch->position)
	    {
		case POS_SLEEPING : rand_to_char(ch,
	...
	"You clench your stomach as you have nightmares about the fabled Rott Worm.\n\r");
			send_to_char("You SCREAM and wake up, sweat running down your forehead.\n\r",ch);
			ch->position = POS_RESTING;
			break;
		case POS_RESTING :
		case POS_SITTING :
			send_to_char("You stand up, screaming in fight as the monsters loom around you.\n\r",ch);
		case POS_FIGHTING :
		case POS_STANDING :
			send_to_char("As the monsters close in around you, you scream for your life and run.\n\r",ch);
		    exec_fear(ch);
	    }
	}
...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    if (get_eq_char (ch, WEAR_SECONDARY) && number_percent() < get_skill(ch,gsn_dual_wield))
    {
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, bool secondary )
{
    ...
    if (dt == gsn_circle)
	thac0 -= 9 * (100 - get_skill(ch,gsn_circle));
...
*/
	 ...
	 */
            ...
            && IS_SET(ch->in_room->room_flags,ROOM_ARENA) )
	 {
              ...
              sprintf(buf,"%s has defeated %s in honorable combat!\n\r",
                      ch->name,victim->name);
                      do_info_char(ch,buf); 
	              ch->pcdata->awins += 1;
...
for(d = descriptor_list; d != NULL; d = d->next)
 {
	 ...
	 d->character->gladiator = NULL;
       ...
       */
         ...
         update_pos( victim );
         fight_in_progress = FALSE; /* clear the arena */
         return TRUE;
	...
	gain_exp( victim, (2 * (exp_per_level(victim,victim->pcdata->points)
	  ...
	  { 
	    ...
	    sprintf(log_buf,"%s was slain by %s",
	    (IS_NPC(victim) ? victim->short_descr: victim->name),
	    (IS_NPC(ch) ? ch->short_descr: ch->name));
	    if(!IS_NPC(victim)&&!IS_NPC(ch))
	    sprintf(log_buf,
	    "FATALITY! %s has been killed by %s",victim->name, ch->name);
            do_info_char(ch,log_buf);
          }
...
/*	if (IS_SET(victim->act,ACT_TRAIN)
	...
	{
	    send_to_char("I don't think Ao would approve.\n\r",ch);
	    return TRUE;
	}
*/
	if (!IS_NPC(ch))
	...
	else
	{
	    ...
	    {
		send_to_char("Become a Pkiller if you want to kill players.\n\r",ch);
		return TRUE;
	    }

	    if (!IS_SET(victim->act,PLR_PKILLER))
	    {
		send_to_char("They aren't a Pkiller, leave them alone.\n\r",ch);
		return TRUE;
	    }
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
     ...
     */
    if ( IS_NPC(victim)
    ||   IS_SET(victim->in_room->room_flags,ROOM_ARENA) /* <- ADD THIS! */
    ...
    sprintf(buf,"%s has engaged %s in MORTAL KOMBAT!!!",ch->name,victim->name);
    do_info_char(ch,buf);
...
void make_corpse( CHAR_DATA *ch )
{
    ...
    OBJ_DATA *obj_next;
    char *name;
    ROOM_INDEX_DATA *location; /* Morgue */
    location = get_room_index ( ROOM_VNUM_MORGUE ); /*morgue */
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
	...
	return;
	 ...
	 if ( dam ==   0 ) { vs = "`0miss``";	vp = "`0misses``";      }
    ...
    else if ( dam <=  20 ) { vs = "`5wound``";	vp = "`5wounds``";	}
    else if ( dam <=  24 ) { vs = "`5maul``";   vp = "`5mauls``";	}
    else if ( dam <=  28 ) { vs = "`5decimate``";vp = "`5decimates``";	}
    else if ( dam <=  32 ) { vs = "`5devastate``";vp = "`5devastates``";}
    else if ( dam <=  36 ) { vs = "`5maim``";	vp = "`5maims``";	}
			     ...
			     vp = "`!=== OBLITERATES ===``";		}
    else if ( dam <= 125)  { vs = "`9>>> ANNIHILATE <<<``";
			     vp = "`9>>> ANNIHILATES <<<``";		}
    else if ( dam <= 150)  { vs = "`@<<< ERADICATE >>>``";
			     vp = "`@<<< ERADICATES >>>``";		}
    else                   { vs = "do `8UNSPEAKABLE`` things to";
			     vp = "does `8UNSPEAKABLE`` things to";     }
...
/*
 * This is the old version of slay kept for reference.
 *
...
void do_slay( CHAR_DATA *ch, char *argument )
{
    ...
    char arg[MAX_INPUT_LENGTH];
    char buf[MAX_STRING_LENGTH]; 
    OBJ_DATA *dahead; /* The head of course */
    char *name; /* To hold victims name */
    ...
    act( "You point your finger at $M and rend life from $S body.",  ch, NULL, victim, TO_CHAR    );
    act( "$n points $s finger at you and you hear the demons of hell chant your name.", ch, NULL, victim, TO_VICT    );
    act( "You hear demons chanting the name of $N, as $n giggles in glee.",  ch, NULL, victim, TO_NOTVICT );


    dahead = create_object(get_obj_index(12), 0); /* Make the head */

    if (dahead == NULL) /* If rom fucked up, return */
     return;

    name = IS_NPC(victim) ? victim->short_descr : victim->name; /* Insert victims name */
    dahead->timer = -1; /* No decay */
/*  dahead->wear  = 65537 */ /* float,take */
    sprintf( buf, dahead->short_descr, name ); /* Put victims name in buf*/
    free_string( dahead->short_descr ); /* Clear short desc */
    dahead->short_descr = str_dup( buf ); /*put ours in */
    sprintf( buf, dahead->description, name ); /* same. but long desc */
    free_string( dahead->description );
    dahead->description = str_dup( buf );
    obj_to_char(dahead, ch); /* Head goes to the slayer */
    act( "$n's head is sliced from $s dead body!" ,victim, NULL, NULL, TO_ROOM ); /* Announce the slaying to the room */
    raw_kill( victim ); /*  The slay part =p */
    return; /*  We outa here! */
}
...
void exec_fear(CHAR_DATA *ch)
{
   ...
   rand_to_char(ch,
	"The monsters slash `#DEMOLISHES`` you.",
	"The nightmares attack `!DEVASTATES`` you.",
	"The beasts bite `5mauls`` you.",
	"The Rott Worm `!DEVASTATES`` you as it bursts from your stomach.",
	"The catoblepas `9*** ANNIHILATES ***`` you.");

   send_to_char("\n\rThe really did `0HURT``!\n\rYou flee from combat.\n\r",ch);
   ...
   ch->in_room = now_in;
   if (!IS_NPC(ch))
	send_to_char("You scream and run away in terror!\n\r",ch);
}
