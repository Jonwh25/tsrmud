...
const struct olc_help_type help_table[] =
{
    ...
    {"act2", act2_flags, "More Mobile attributes."},
...
/* ROM specific bits: */
    ...
    {"gate", gate_flags, "Portal/Gate types."}, 
    ...
    {"guard_dir", guard_direction, "Mobile guard direction behaviour."},
    {"action", guard_action, "Mobile guard action behaviour."},
...
REDIT(redit_show)
{
    ...
    sprintf(buf, "Mana Rate:  [%d]\n\rHeal Rate:  [%d]\n\r",
	     pRoom->mana_rate, pRoom->heal_rate);
    strcat(buf1, buf);
   ...
   if (pRoom->extra_descr)
	...
	if ((pexit = pRoom->exit[door]))
	{
	    ...
	    /*
	       Format up the exit info. Capitalize all flags that are not part
	       of the reset info.
	    */
...
bool change_exit(CHAR_DATA * ch, char *argument, int door)
{
    ...
    */
	...
	if (!pRoom->exit[door])
	    ...
	    pRoom->exit[door]->orig_door=door; /*Stormy*/
	...
	*/
	...
	rev = rev_dir[door];
       ...
       /* Fix against setting exit flags before the exit exists-Stormy*/
	if(pToRoom == NULL)
	{
		send_to_char("You have to create the exit first.\n\r",ch);
	}
       
        /*
	 * Bugfix against doors that are one way donated to 
	 * TSR's OLC for ROM 2.4 by none other than
         * Hans Hvidsten Birkeland <hansbi@ifi.uio.no>
         */
...
REDIT(redit_mreset)
{
    ...
    if (arg[0] == '\0' || !is_number(arg))
    {
	send_to_char("Syntax:  mreset <vnum> <max #> <min #>\n\r", ch);
	return FALSE;
    }
    ...
    */
    ...
    pReset->arg2 = is_number(argument) ? atoi(argument) : MIN_MOB;
    pReset->arg3 = pRoom->vnum;
    pReset->arg4 = is_number(argument) ? atoi(argument) : MIN_MOB;
    ...
    */
    ...
    sprintf(output, "%s (%d) has been loaded and added to resets.\n\r"
	    "There will be a maximum of %d in the world.\n\r"
	    "There will be %d loaded to this room.\n\r",
...
REDIT(redit_oreset)
{
	...
	send_to_char("        -<obj_name> <number>   = into obj\n\r", ch);
	...
	*/
	...
	pReset->arg3 = to_obj->pIndexData->vnum;
        pReset->arg4 = is_number(argument) ? atoi(argument) : MIN_OBJ;
	add_reset(pRoom, pReset, 0 /* Last slot */ );
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
	...
	case ITEM_CONTAINER:
	    sprintf(buf,
		    "[v0] Weight Compacity: [%d]\n\r"
		    "[v1] Flags:  [%s]\n\r"
		    "[v2] Key Vnum if Locked: [%d]\n\r"
		    "[v3] Max Weight: [%d]\n\r"
	...
	case ITEM_PORTAL:
	    sprintf(buf,
		   "[v0] Charges of portal: [%d]\n\r"
		   "[v1] Exit flags:        [%s]\n\r"
		   "[v2] Gate flags:        [%s]\n\r"
		   "[v3] Room to connect to (Destination): [%d]\n\r",
		   obj->value[0],
		   flag_string( exit_flags, obj->value[1] ), 
		   flag_string( gate_flags, obj->value[2] ), 
		   obj->value[3]);
	    add_buf( buffer, buf );
	    break;
	...
	case ITEM_FURNITURE:
	    sprintf(buf,
		    "[v0] Max number of people: [%d]\n\r"
		    "[v1] Total weight it can support: [%d]\n\r"
		    "[v2] Furniture Flags: [%s]\n\r"
		    "[v3] Healing Bonus/Penalty: [%d]\n\r"
		    "[v4] Mana Bonus/Penalty: [%d]\n\r",
		    obj->value[0],
		    obj->value[1],
...
/* OLC              do_help( ch, "liquids" );    */
	...
	case ITEM_PORTAL:
		...
		default:
		    ...
		    return FALSE;
		case 0:
		    send_to_char("CHARGES OF PORTAL SET.\n\r\n\r", ch); 
		    pObj->value[0] = atoi(argument); break;
		    break;
		case 1:
		  ...
		  if ( value != NO_FLAG)
		  {
		    ...
		    break;
		case 2:
		   value = flag_value( gate_flags, argument );
		   if ( value != NO_FLAG)
		   {
		   ...
		   break;
		case 3:
		    send_to_char("ROOM CONNECTED TO SET.\n\r\n\r", ch); 
		    pObj->value[3] = atoi(argument); break;
		    break;
	    }
	    break;
	...
	case ITEM_FURNITURE:
		...
		default:
		    ...
		    return FALSE;
		case 0:
		    send_to_char("MAX # OF PEOPLE SET.\n\r\n\r", ch);
		    pObj->value[0] = atoi(argument);
		    break;
		case 1:
	...
	case ITEM_BOAT:
	    ...
	    {
		default:
		    do_help(ch, "ITEM_BOAT");
		    return FALSE;
		case 4:
		    send_to_char("TOTAL MOVE TOGGLED.\n\r\n\r", ch);
		    pObj->value[4] = atoi(argument);
		    break;
	    }
	    break;
...
OEDIT(oedit_show)
{
	...
	if (cnt == 0)
	{
	    add_buf( buffer, "Number Modifier Affects\n\r");
	    add_buf( buffer, "------ -------- -------\n\r");
	}
	sprintf(buf, "[%4d] %-8d %s\n\r", cnt,
		paf->modifier,
...
OEDIT(oedit_extra)              /* Moved out of oedit() due to naming conflicts
				   -- Hugin */
{
    OBJ_INDEX_DATA *pObj;
    int value;
...
OEDIT(oedit_wear)               /* Moved out of oedit() due to naming conflicts
				   -- Hugin */
{
    OBJ_INDEX_DATA *pObj;
    int value;
...
OEDIT(oedit_type)               /* Moved out of oedit() due to naming conflicts
				   -- Hugin */
{
    OBJ_INDEX_DATA *pObj;
    int value;
...
/* ROM values: */
    ...
    sprintf(buf, "Guard:       [%s]\n\r",
	    flag_string(guard_direction, pMob->guard_dir));
    add_buf(buffer, buf);

    sprintf(buf, "Action:      [%s]\n\r",
	    flag_string(guard_action, pMob->guard_action));
    add_buf(buffer, buf);

    sprintf(buf, "Obj Pass:    [%d]\n\r",
	    pMob->pass_vnum);
    add_buf(buffer, buf);
...
MEDIT(medit_sex)                /* Moved out of medit() due to naming conflicts
				   -- Hugin */
{
    MOB_INDEX_DATA *pMob;
    int value;
...
MEDIT(medit_act)                /* Moved out of medit() due to naming conflicts
				   -- Hugin */
{
    MOB_INDEX_DATA *pMob;
    int value;
...
MEDIT(medit_affect)             /* Moved out of medit() due to naming conflicts
				   -- Hugin */
{
    MOB_INDEX_DATA *pMob;
    int value;
...
MEDIT(medit_ac)
{
    ...
    do                          /* So that I can use break and send the syntax
				   in one place */
    {
	if (argument[0] == '\0')
	    break;
...
if ((dtype = attack_lookup(argument))==0)
{
...
MEDIT(medit_guard)
{
	...
	if ((value = flag_value(guard_direction, argument)) != NO_FLAG)
	{
	    pMob->guard_dir ^= value;
	    send_to_char("Guard direction toggled.\n\r", ch);
	    return TRUE;
	}
    }

    send_to_char("Syntax: guard [flags]\n\r"
		 "Type '? guard' for a list of flags.\n\r", ch);
    return FALSE;
}
...
MEDIT(medit_action)
{
	...
	if ((value = flag_value(guard_action, argument)) != NO_FLAG)
	{
	    pMob->guard_action ^= value;
	    send_to_char("Guard action toggled.\n\r", ch);
	    return TRUE;
	}
    }

    send_to_char("Syntax: action [flags]\n\r"
		 "Type '? action' for a list of flags.\n\r", ch);
    return FALSE;
}
...
MEDIT(medit_gvnum)
{
    ...
    if (argument[0] == '\0' || !is_number(argument))
    {
	send_to_char("Syntax:  gvnum [number]\n\r", ch);
	return FALSE;
    }

    pMob->pass_vnum = atoi(argument);

    send_to_char("Object player must have to get by is set.\n\r", ch);
    return TRUE;
}
  /*
   *  NEW act2 flag was needed...so well..I coded it thanks
   *  to a little help from whiplash. :) -Beowolf
   */
