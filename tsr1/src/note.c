...
void do_unread(CHAR_DATA *ch)
{
    ...
    if ((count = count_spool(ch,letter_list)) > 0)
    {
	found = TRUE;
	sprintf(buf,"There %s %d new letters%s waiting to be read.\n\r",
	    count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
        send_to_char(buf,ch);
    }
...
{
    parse_note(ch,argument,NOTE_LETTER);
}
...
void load_notes(void)
{
    ...
    load_thread(LETTER_FILE,&letter_list,NOTE_LETTER, 0);
}
...
bool is_note_to( CHAR_DATA *ch, NOTE_DATA *pnote )
{
    ...
    if ( is_exact_name( "everyone", pnote->to_list ) )
	return TRUE;
    ...
    if ( IS_BUILD(ch) && is_exact_name( "builder", pnote->to_list ) )
	return TRUE;
...
bool hide_note (CHAR_DATA *ch, NOTE_DATA *pnote)
{
	...
	case NOTE_NEWS:
	    ...
	    break;
	case NOTE_LETTER:
	    last_read = ch->pcdata->last_letter;
	    break;
    }
...
void update_read(CHAR_DATA *ch, NOTE_DATA *pnote)
{
    ...
    switch (pnote->type)
    {
	    ...
	    ch->pcdata->last_news = UMAX(ch->pcdata->last_news,stamp);
            break;
        case NOTE_LETTER:
	    ch->pcdata->last_letter = UMAX(ch->pcdata->last_letter,stamp);
            break;
    }
}
...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
	...
	if (!vnum)
	{
		...
		case NOTE_NEWS:
		    ...
		    break;
		case NOTE_LETTER:
		    send_to_char("There are no letters for you.\n\r",ch);
		    break;
	    }
	}
	return;
	...
	switch(type)
	{
		...
		break;
	    case NOTE_LETTER:
		ch->pcdata->last_letter = current_time;
		break;
	}
	return;
    }
	...
	{
	    send_to_char(
		"You need to provide a recipient (name, every builder, or immortal).\n\r",
		ch);
	    return;
	}
