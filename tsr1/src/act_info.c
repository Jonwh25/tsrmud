...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if ( victim->in_ship )		strcat( buf, "[`$Aboard Ship``] " );
    if ( victim->in_wagon )		strcat( buf, "[`$Aboard Wagon``] " );
    ...
    if ( IS_AFFECTED(victim, AFF_HIDE)        ) strcat( buf, "(`hHide``) "       );
...
void do_bmotd(CHAR_DATA *ch, char *argument)
{  
    do_help(ch,"bmotd");
}
...
/* 
 * Taken out and replaced with New version
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"You have %d practices, %d training sessions and %d learning sessions\n\r",
	ch->practice, ch->train, ch->pcdata->learn);
    send_to_char( buf, ch );
    ...
    sprintf( buf, "You have %d gold in the bank and need %d to level\n\r", ch->bank,
    ((ch->level + 1) * exp_per_level(ch,ch->pcdata->points) - ch->exp));
    send_to_char (buf, ch );
	...
	if (!IS_SET(ch->act, PLR_QUESTOR))
	{
	    ...
	    {
		sprintf(buf, 
			"You can quest again in less than a minute.\n\r");
		send_to_char(buf, ch);
	    }
	}
        else if (ch->countdown > 0)
        {
	    sprintf(buf, "Time left on quest: %d\n\r",ch->countdown);
	    send_to_char(buf, ch);
	}
     }
 if(!IS_NPC(ch))
 {
 sprintf(buf,"arena wins: %d arena losses: %d\n\r",
...
void do_help( CHAR_DATA *ch, char *argument )
{
	...
	if ( is_name( argall, pHelp->keyword ) )
	{
		...
		add_buf(output,
    "\n\r============================================================\n\r\n\r");
	    if ( pHelp->level >= 0 && str_cmp( argall, "bmotd" ) )
	    {
    ...
    for ( ;; )
    {
        ...
        else
        {
			...
			if (!str_prefix(arg,"clan"))
			    ...
			    {
                        	send_to_char(
                            	"That's not a valid race, class, or sect.\n\r",
				   ch);
                            	return;
			    }
                        }
...
"`1==========================================================================``\n\r\n\r", ch );
	...
	default: break;
            {
                case MAX_LEVEL - 0 : class = "`8ADM``";     break;
                case MAX_LEVEL - 1 : class = "`8IMP``";     break;
                case MAX_LEVEL - 2 : class = "`9CRE``";     break;
                case MAX_LEVEL - 3 : class = "`4DEI``";     break;
                case MAX_LEVEL - 4 : class = "`5GOD``";     break;
                case MAX_LEVEL - 5 : class = "`1IMM``";     break;
                case MAX_LEVEL - 6 : class = "`!DEM``";     break;
                case MAX_LEVEL - 7 : class = "`@ANG``";     break;
                case MAX_LEVEL - 8 : class = "`3AVA``";     break;
                case MAX_LEVEL - 9 : class = "`#BLD``";     break;
                case LEVEL_HERO    : class = "`2HRO``";     break;
            }
	}
  ...
  sprintf( buf, 
"`@[`1%3d`` ```#%11s`` `$%s`` `8%c```@]`` %s%s%s%s%s%s%s%s\n\r",
            wch->level,
	    ...
	    wch->clan ? clan_table[wch->clan].who_name: "");
	add_buf(output,buf);
    }
    ...
    if (pMatch >= 0  )
    {
	sprintf( buf2, "\n\r`3Visible player(s):`` `0%d``\n\r", pMatch );
	add_buf (output, buf2);
    }
    
    if (iMatch >= 0  )
    {
	sprintf( buf2, "`3Visible Immortal(s):`` `0%d``\n\r", iMatch );
	add_buf (output, buf2);
    }
    ...
    if (max_on == count)
      sprintf(buf2, "`3Total on Today:`` `0%d``\n\r", count);
    else
      sprintf(buf2, "`3Total on Today:`` `0%d``\n\r", max_on);
    add_buf (output, buf2);
...
{
    sprintf(c,"%s %s %s",color_table[b],a,ANSI_NORMAL);  
    send_to_char(c,ch);	
}
 		...
 		if (strcasecmp(cbuf,"list")==0) 
 		   ...
 		   while(color_table[i])
 		   	...
 		   	if(strcasecmp(cbuf,"show")==0)
 		   	  {
 		   	  send_to_char("Below is a list of the current Colors that you have selected:\n\r",ch);
 		   	  send_to_char(" ",ch); 		   	  
 		   	  send_color_status(ch,"1 charmed mobs ",cbuf,ch->pcdata->color_charmed);
 		   	  send_to_char(" ",ch); 		   	  
 		   	  send_color_status(ch,"2 devotion         ",cbuf,ch->pcdata->color_dev);
 		   	  send_to_char(" ",ch); 		   	  
 		   	  send_color_status(ch,"3 prompt       ",cbuf,ch->pcdata->color_prompt);
 		   	  send_to_char(" ",ch); 		   	  
 		   	  send_color_status(ch,"4 hidden mobs  ",cbuf,ch->pcdata->color_hidden);
 		   	  send_to_char("\n\r",ch); 		   	   		   	  
 		   	  send_to_char(" ",ch); 		   	  
 		   	  send_color_status(ch,"5 invis mobs   ",cbuf,ch->pcdata->color_invis);
 		   	  send_to_char(" ",ch); 		   	  
 		   	  send_color_status(ch,"6 ooc channel  ",cbuf,ch->pcdata->color_ooc);
 		   	  send_to_char(" ",ch); 		   	  
 		   	  send_color_status(ch,"7 reply        ",cbuf,ch->pcdata->color_reply);
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"8 say          ",cbuf,ch->pcdata->color_say);              
 		   	  send_to_char("\n\r",ch); 		   	   		   	  
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"9 tell         ",cbuf,ch->pcdata->color_tell);
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"10 wizi mobs   ",cbuf,ch->pcdata->color_wizi);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"11 shout       ",cbuf,ch->pcdata->color_shout);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"12 auction     ",cbuf,ch->pcdata->color_auction);              
 		   	  send_to_char("\n\r",ch); 		   	                
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"13 yell        ",cbuf,ch->pcdata->color_yell);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"14 gossip      ",cbuf,ch->pcdata->color_gossip);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"15 grats       ",cbuf,ch->pcdata->color_grats);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"16 info        ",cbuf,ch->pcdata->color_info);              
 		   	  send_to_char("\n\r",ch); 		   	                
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"17 music       ",cbuf,ch->pcdata->color_music);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"18 quote       ",cbuf,ch->pcdata->color_quote);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"19 immortal    ",cbuf,ch->pcdata->color_immortal);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"20 build       ",cbuf,ch->pcdata->color_build);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"21 newbie       ",cbuf,ch->pcdata->color_newbie);              
 		   	  send_to_char(" ",ch);              
                          send_color_status(ch,"22 Admin       ",cbuf,ch->pcdata->color_admin);              
 		   	  send_to_char("`^``\n\r",ch); 		   	                
 		   	  }
 		   	   else
 		   	if(strcasecmp(cbuf,"set")==0)
 		   	 { /* if cbuf */
 		   	 argument=one_argument(argument,cbuf);
 		   	 argument=one_argument(argument,cbuf1);
 		   	 i=0;
 		   	 if (isdigit(cbuf[0]) && isalpha(cbuf1[0]))
 		   	   {
 		   	 		while (color_name[i]) {
 		   	 				if (!str_cmp(color_name[i],cbuf1)) {
 		   	 					b=i; 
 		   	 					a=atoi(cbuf); 		   	 				
 		   	 					}
 		   	 					i++;
 		   	     ...
 		   	     if (isdigit(cbuf1[0]) && isalpha(cbuf[0]))
 		   	     {
 		   	 		while (color_name[i]) {
 		   	 				if (!str_cmp(color_name[i],cbuf)) {
 		   	 					b=i; 
 		   	 					a=atoi(cbuf1);
 		   	 					}
 		   	 					i++;
 		   	 ...
 		   	 if ((b<i) && (b>=0)) /* check for valid color */
 		   	    {
			    switch(a) { /* switch a */
			 	case 1:  ch->pcdata->color_charmed=b;send_to_char("`Ccharmed mobs``\n\r",ch); break;
			 	case 2:  ch->pcdata->color_dev=b;send_to_char("`FDevotion Channel is set.``\n\r",ch); break;
			 	case 3:  ch->pcdata->color_prompt=b;send_to_char("`Pprompt``\n\r",ch); break;
			 	case 4:  ch->pcdata->color_hidden=b;send_to_char("`Hhidden``\n\r",ch); break;
			        case 5:  ch->pcdata->color_invis=b;send_to_char("`iinvis``\n\r",ch); break;
			 	case 6:  ch->pcdata->color_ooc=b;send_to_char("`Oooc``\n\r",ch); break;
			 	case 7:  ch->pcdata->color_reply=b;send_to_char("`Rreply``\n\r",ch); break;
			 	case 8:  ch->pcdata->color_say=b;send_to_char("`Ssay``\n\r",ch); break;
			 	case 9:  ch->pcdata->color_tell=b;send_to_char("`Ttell``\n\r",ch); break;			 	
			 	case 10: ch->pcdata->color_wizi=b;send_to_char("`Wwizi``\n\r",ch); break;			 	
			 	case 11: ch->pcdata->color_shout=b;send_to_char("`Xshout``\n\r",ch); break;			 	
			 	case 12: ch->pcdata->color_auction=b;send_to_char("`Yauction``\n\r",ch); break;			 				 	
			 	case 13: ch->pcdata->color_yell=b;send_to_char("`yyell``\n\r",ch); break;			 	
			 	case 14: ch->pcdata->color_gossip=b;send_to_char("`Ggossip``\n\r",ch); break;			 	
			 	case 15: ch->pcdata->color_grats=b;send_to_char("`ggrats``\n\r",ch); break;			 	
			 	case 16: ch->pcdata->color_info=b;send_to_char("`Iinfo``\n\r",ch); break;			 	
			 	case 17: ch->pcdata->color_music=b;send_to_char("`Mmusic``\n\r",ch); break;			 	
			 	case 18: ch->pcdata->color_quote=b;send_to_char("`Qquote``\n\r",ch); break;
			 	case 19: ch->pcdata->color_immortal=b;send_to_char("`Aimmortal``\n\r",ch); break;
			 	case 20: ch->pcdata->color_build=b;send_to_char("`Bbuild``\n\r",ch); break;
			 	case 21: ch->pcdata->color_newbie=b;send_to_char("`nNewbie Channel Color set``\n\r",ch); break;
			 	case 22: ch->pcdata->color_admin=b;send_to_char("`ZAdmin Channel Color set``\n\r",ch); break;
			 	default: send_to_char("Syntax: color set <color number> <channel number>\n\r",ch); break;
			        } /* switch a */
			    } /* if (b...) */
...
/* NEW SCORE FUNCTION BY BEOWOLF AND THE LOVELY NIKKI */
...
"| Title: %-66s |\n\r",
   ...
   send_to_char( buf, ch );
    ...
    sprintf( buf,
"| Level: %-6d Age: %-6d Hours: %-6d QuestPoints: %-20d |\n\r",
	ch->level, 
        get_age(ch),
        ( ch->played + (int) (current_time - ch->logon) ) / 3600,
	ch->questpoints);
...
"| Race: %-17s Gender: %-17s Class: %-16s |\n\r", 
    ...
    sprintf( buf,
"| Exp.: %-7d Xp to Level: %-4d Practices: %-4d Trains: %-4d Learns: %-4d |\n\r",
        ch->exp,    
	((ch->level + 1) * exp_per_level(ch,ch->pcdata->points) - ch->exp),
	ch->practice, ch->train, ch->pcdata->learn);
   send_to_char( buf, ch );
    ...
    sprintf( buf,
"| Hitpoints: %5d/%-8d Mana: %5d/%-10d Movement: %5d/%-8d |\n\r",
	ch->hit,  ch->max_hit,
	ch->mana, ch->max_mana,
	ch->move, ch->max_move);
   send_to_char( buf, ch );
...
ch);
    ...
    sprintf( buf,
"| Str: %2d (%2d)   Dex: %2d (%2d)                Pierce: %5d      Bash: %5d |\n\r",
	ch->perm_stat[STAT_STR],
	get_curr_stat(ch,STAT_STR),
	ch->perm_stat[STAT_DEX],
	get_curr_stat(ch,STAT_DEX),
   ...
   send_to_char( buf, ch );
    ...
    sprintf( buf,
"| Int: %2d (%2d)   Wis: %2d (%2d)                 Slash: %5d    Exotic: %5d |\n\r",
	ch->perm_stat[STAT_INT],
	get_curr_stat(ch,STAT_INT),
	ch->perm_stat[STAT_WIS],
	get_curr_stat(ch,STAT_WIS),
   ...
   send_to_char( buf, ch );
    ...
    sprintf( buf,
"| Con: %2d (%2d)                              Hitroll: %5d   Damroll: %5d |\n\r",
	ch->perm_stat[STAT_CON],
	get_curr_stat(ch,STAT_CON), 
        GET_HITROLL(ch), GET_DAMROLL(ch) );
   send_to_char( buf, ch );
    ...
    sprintf( buf,
"| Items: %5d / %-5d     Weight: %8d / %-8d                      |\n\r",
	ch->carry_number, can_carry_n(ch),
	get_carry_weight(ch) /10, can_carry_w(ch) /10 );
   send_to_char( buf, ch );

    sprintf( buf,
"| Gold: %-10d             Silver: %-10d          Bank: %-10d |\n\r",
        ch->gold, 
        ch->silver,
        ch->bank);
   send_to_char( buf, ch ); 
    ...
    sprintf( buf,
"| Arena Wins: %-3d        Arena Losses: %-3d      Wimpy: %-4d                 |\n\r",
        ch->pcdata->awins, ch->pcdata->alosses, ch->wimpy );
   send_to_char( buf, ch );

    if(ch->gladiator != NULL)
     {
       sprintf( buf,
"| Arena Bet in Gold: %d      Arena Bet on: %s                               |\n\r",
        ch->pcdata->plr_wager, ch->gladiator->name);
     }
...
"|  You are ", ch );
    ...
    if ( IS_IMMORTAL(ch))
    {
      ...
      if (ch->invis_level)
      {
        sprintf( buf, "  Invisible: level %-3d",ch->invis_level);
        send_to_char(buf,ch);
      }

      if (ch->incog_level)
      {
	sprintf(buf,"  Incognito: level %-3d",ch->incog_level);
	send_to_char(buf,ch);
      }
      send_to_char("                                      |\n\r",ch);
    }
...
/* Fun and funky display routines! :) */

void rand_to_char(CHAR_DATA *ch, char *str1, char *str2, char *str3, char *str4, char *str5) 
{
    switch(dice(1,5))
    {
	case 1 : send_to_char(str1, ch);
		 break;
	case 2 : send_to_char(str2, ch);
		 break;
	case 3 : send_to_char(str3, ch);
		 break;
	case 4 : send_to_char(str4, ch);
		 break;
	case 5 : send_to_char(str5, ch);
		 break;
	default:bug("BUG: rand_to_char: number out of range.",0);
		 break;
    }
}
...
void do_pkill(CHAR_DATA *ch, char *argument)
{
   ...
   if (ch->pcdata->confirm_pkill )
   {
    ...
    wiznet("$N is contemplating becomeing a pkiller.",ch,NULL,0,0,get_trust(ch));
}
