...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
	...
	case TO_VULN:
	    ...
	    break;
	case TO_FLAG2:
	    SET_BIT(ch->flag2, paf->bitvector);
	    break;
	case TO_FLAG3:
	    SET_BIT(ch->flag3, paf->bitvector);
	    break;
	case TO_FLAG4:
	    SET_BIT(ch->flag4, paf->bitvector);
	    break;
	}
    }
    else
    {
        ...
        case TO_VULN:
            ...
            break;
        case TO_FLAG2:
            REMOVE_BIT(ch->flag2, paf->bitvector);
            break;
        case TO_FLAG3:
            REMOVE_BIT(ch->flag3, paf->bitvector);
            break;
        case TO_FLAG4:
            REMOVE_BIT(ch->flag4, paf->bitvector);
            break;
        }
	mod = 0 - mod;
    }
...
void affect_check(CHAR_DATA *ch,int where,int vector)
{
	...
	if (paf->where == where && paf->bitvector == vector)
	{
	    switch (where)
	    {
		    ...
		    break;
	        case TO_FLAG2:
		    SET_BIT(ch->flag2, vector);
		    break;
	        case TO_FLAG3:
		    SET_BIT(ch->flag3, vector);
		    break;
	        case TO_FLAG4:
		    SET_BIT(ch->flag4, vector);
		    break;
	    }
	    return;
	}
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    /* BUG FIX from ROM mailing list */

    if ( !IS_NPC( ch ) && ( obj = get_eq_char( ch, WEAR_WIELD ) ) != NULL )
       extract_obj( obj );
	...
	if ( wch->reply == ch )
    	...
    	if (IS_NPC(wch) && wch->mind == ch)
    	    wch->mind = NULL;

	if ( ch->mprog_target == wch )   /* MOBPROGS */
 	    wch->mprog_target = NULL;    /* MOBPROGS */
    }
...
/*
 * Try and fix the Mob Guardian Code to see this field...
 * Pray with me please.
 */

char *guard_bit_name( int guard_dir)
{
    static char buf[512];

    buf[0] = '\0';

    if (guard_dir & MOB_GUARD_DIR_NORTH ) strcat(buf, " guard_north");
    if (guard_dir & MOB_GUARD_DIR_SOUTH ) strcat(buf, " guard_south");
    if (guard_dir & MOB_GUARD_DIR_EAST  ) strcat(buf, " guard_east");
    if (guard_dir & MOB_GUARD_DIR_WEST  ) strcat(buf, " guard_west");
    if (guard_dir & MOB_GUARD_DIR_UP    ) strcat(buf, " guard_up");
    if (guard_dir & MOB_GUARD_DIR_DOWN  ) strcat(buf, " guard_down");

    return (buf[0] != '\0' ) ? buf+1 : "none";
}

char *action_bit_name( int guard_action)
{
    static char buf[512];

    buf[0] = '\0';

    if (guard_action & GUARD_ACTION_STOP_CHAR ) strcat(buf, " stop");
    if (guard_action & GUARD_ACTION_ATTACK ) strcat(buf, " attack");
    if (guard_action & GUARD_ACTION_TELEPORT ) strcat(buf, " teleport");
    if (guard_action & GUARD_ACTION_CHECK_ITEM ) strcat(buf, " item");
    if (guard_action & GUARD_ACTION_CHECK_CLASS ) strcat(buf, " check_class");
    if (guard_action & GUARD_ACTION_CHECK_RACE ) strcat(buf, " check_race");
    if (guard_action & GUARD_ACTION_CHECK_SECT ) strcat(buf, " check_sect");
    if (guard_action & GUARD_ACTION_CHECK_LEVEL ) strcat(buf, " check_level");
    if (guard_action & GUARD_ACTION_CHECK_AGE ) strcat(buf, " check_age");
    if (guard_action & GUARD_ACTION_CHECK_ALIGN ) strcat(buf, " check_align");
...
char *act2_bit_name( int act2_flags)
{
    ...
    if (act2_flags & ACT2_CHALLENGER ) strcat(buf, " challenger ");
    if (act2_flags & ACT2_CHALLENGED ) strcat(buf, " challenged ");
    if (act2_flags & ACT2_MOUNTABLE )  strcat(buf, " mountable ");
    if (act2_flags & ACT2_TEACHER )  strcat(buf, " teacher ");
...
/*
 * And now back to our regular schedualed program.
 * heh. I said program.
 */
...
char *off_bit_name(int off_flags)
{
    ...
    if (off_flags & OFF_CIRCLE  	) strcat(buf, " circle");
    ...
    if (off_flags & MOB_IS_GUARD	) strcat(buf, " mob_is_guard");
 ...
 * (from get_char_world)
 *
 * Put this in your handler.c and define it in merc.h just the same way
 * as get_char_world is defined.
 *
...
bool can_see_ship( CHAR_DATA *ch, CHAR_DATA *victim )
{
    if ( victim->in_ship )
	return FALSE;
...
bool can_see_wagon( CHAR_DATA *ch, CHAR_DATA *victim )
{
    if ( victim->in_wagon )
	return FALSE;
