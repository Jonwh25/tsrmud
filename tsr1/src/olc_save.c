...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    fprintf( fp, "%s ",           fwrite_flag( pMobIndex->off_flags,  buf ) );
	    ...
	    /*
	     * Mob Guardians. This took me a while to figure out.
	     * This idea was fixed when I, Beowolf, mailed merc-l
	     * and got the idea for a if IS_SET statement from
	     * Jeff Washburn.
	     */
   
    if (IS_SET ( pMobIndex->off_flags, MOB_IS_GUARD ) )
    {
    fprintf( fp, "%s ",           fwrite_flag( pMobIndex->guard_dir,  buf ) );
    fprintf( fp, "%s ",           fwrite_flag( pMobIndex->guard_action, buf ) );
    fprintf( fp, "%d ",           pMobIndex->pass_vnum );
    }
   	    /* 
	     * End Mob Guardians saving
	     */
...
void save_object( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
    ...
    char buf[MAX_STRING_LENGTH], wtype[20] ;
...
{
case WEAPON_EXOTIC : strcpy (wtype,"exotic") ; break ;
case WEAPON_SWORD : strcpy(wtype,"sword") ; break ;
case WEAPON_DAGGER : strcpy(wtype,"dagger") ; break ;
case WEAPON_SPEAR : strcpy(wtype,"spear") ; break ;
case WEAPON_MACE : strcpy(wtype,"mace") ; break ;
case WEAPON_AXE : strcpy(wtype,"axe") ; break ;
case WEAPON_FLAIL : strcpy(wtype,"flail") ; break ;
case WEAPON_WHIP : strcpy(wtype,"whip"); break ;
case WEAPON_POLEARM : strcpy(wtype,"polearm") ; break ;
default : strcpy(wtype,"unknown") ; break ;
}
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
    {
		...
		fprintf( fp, "0 " );
			...
			if ( IS_SET( pExit->rs_flags, EX_NOPASS ) )
			{    
			    locks = 3;
		        }	
