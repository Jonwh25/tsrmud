...
pMobIndex->area                 = area_last;               /* OLC */
	...
	pMobIndex->off_flags		= fread_flag( fp ) 
        ...
        /* Additions for guard mobs - Froth */

	if ( IS_SET(pMobIndex->off_flags, MOB_IS_GUARD) )
	{
	pMobIndex->guard_dir			= fread_flag( fp );
	pMobIndex->guard_action		        = fread_flag( fp );
	pMobIndex->pass_vnum 			= fread_number( fp );
	}
	else
	{
...
pMobIndex->sex=fread_number(fp) ;*/
	...
	/* MOB Guardians */	
		else if (!str_prefix(word,"guard"))
		    REMOVE_BIT(pMobIndex->guard_dir,vector);
		else if (!str_prefix(word,"action"))
		    REMOVE_BIT(pMobIndex->guard_action,vector);
		else if (!str_prefix(word,"gvnum"))
		    REMOVE_BIT(pMobIndex->pass_vnum,vector);
	/* End of MOB Guardians. */	
