...
void do_remove( CHAR_DATA *ch, char *argument )
{
    ...
    /* 
     * The Remove all bit of code was donated to TSR by:
     * Christopher S Smith <stoked@iastate.edu>
     */
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    * variables for AUTOSPLIT *
    ...
    {
	act( "$n offers $mself to Ao, who graciously declines.",
	    ch, NULL, NULL, TO_ROOM );
	send_to_char(
	    "Ao appreciates your offer and may accept it later.\n\r", ch );
	return;
    }
    ...
    if (silver == 1)
        send_to_char(
	    "Ao gives you one silver coin for your sacrifice.\n\r", ch );
    else
    {
	sprintf(buf,"Ao gives you %d silver coins for your sacrifice.\n\r",
		silver);
	send_to_char(buf,ch);
    }
    ...
    act( "$n sacrifices $p to Ao.", ch, obj, NULL, TO_ROOM );
...
/* wear object as a secondary weapon */
{
    ...
    {
        send_to_char("You don't know how to use a second weapon.\n\r",ch);
        return;
    }
...
void do_deposit ( CHAR_DATA *ch, char *argument )
{
    ...
    if ( amnt < 0 )
    {
	sprintf( buf, "%s, you are a fucking retard!\n\r", ch->name );
	do_say( banker, buf );
	return;
    }
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
	...
	act( "$n offers $mself to $t, who graciously declines.",
	    ch, clan_table[ch->clan].god, NULL, TO_ROOM );
	sprintf( buf, "%s appreciates your offer and may accept it later.\n\r", 
		 clan_table[ch->clan].god );
	send_to_char( buf, ch );
	return;
    }
    ...
    act( "$n sacrifices $p to $T.", ch, obj,  clan_table[ch->clan].god, 
	TO_ROOM );
    wiznet("$N sends up $p as a burnt offering.",
	   ch,obj,WIZ_SACCING,0,0);
    extract_obj( obj );
...
void do_sheath( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "", ch );
    else if ( obj->wear_loc == WEAR_SECONDARY )
	send_to_char( "", ch );
    else
    {
	send_to_char( "You do not have that item.\n\r", ch );
    ...
    if ( !IS_SET( obj1->wear_flags, ITEM_WEAR_BELT ) )
    {
       	send_to_char( "You need a belt to sheath it in.\n\r", ch );
       	return;
    }
