...
/**************************************************************************
	Ooops...Looks like the mailing list from merc saved us here. Two
bug fixes were posted. below this is the origianal Code and what was posted
to fix this bug:
...
Posted Fix Suggestion:

             if( *ch->desc->pString >= string_space &&  
                 *ch->desc->pString < top_string )    
             {                                              
                 send_to_char( "String cleared!\n\r", ch ); 
...
Wreckie Boy takes Charge and his code is used:

don't think you need to get through that much trouble: just free the 
string and strr_dup( "" ). Free_string( ) will do the work of checking 
the strings in struing space for you...
Here's my fix:
Wreckie.
************************************************************************/
