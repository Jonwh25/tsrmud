...
void do_mpapply( CHAR_DATA *ch, char *argument )
{
  ...
  /* For auto auth if Immortal is not on */
  for ( d = first_descriptor; d; d=d->next )
     if ( d->connected == CON_PLAYING && d->character
        && IS_IMMORTAL(d->character)
        && can_see( victim, d->character) )
     FOUND = TRUE;
  ...
  if (!FOUND)
  {
     ...
     if ( victim->pcdata->authed_by )
        ...
        victim->pcdata->authed_by =  str_dup( "Auto Auth" ) ;
        sprintf( buf, "%s: authorized", victim->name);
        to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE );
        ch_printf_color( victim,
           "\n\r&GNo immortals are online to accept you %s.\n\r"
           "You are authorized to enter The Shadow Realms at the end of "
           "this area.\n\rHowever, you &YMAY&G be required to change your"
           "name later.\n\r", victim->name);
  }
