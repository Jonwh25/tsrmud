...
/* start from last to first to get it in the proper order */
	...
	if ( wch->pcdata->clan )
	{
	  ...
	  invis_str[0] = '\0'; 
	sprintf( buf, "[%-10s %10s] %s%s%s%s%s%s%s%s%s\n\r",
...
void do_finger(CHAR_DATA *ch, char *argument)
{
    ...
    name = capitalize( arg );
    sprintf( strsave, "%s%c/%s", PLAYER_DIR, tolower(arg[0]), name );
    ...
    if ( ch->level < MAX_LEVEL )
    {
       ...
       {
        sprintf(buf,
          "I don't think %s would like that.\n\r",capitalize(arg));
        send_to_char(buf, ch);
        return;
       }
    }
    ...
    sprintf(buf, "%s last logged on at %s", capitalize(arg),
       laston_found ? ctime(&laston) : "Not since this was installed.");
    send_to_char(buf, ch);
